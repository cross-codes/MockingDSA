...
public class PolygonArea implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new PolygonArea(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() throws IOException {
    ...
    for (int j = 1; j < X.length; j++) {
      res += det(X[j], X[(j == X.length - 1) ? 1 : j + 1], Y[j], Y[(j == X.length - 1) ? 1 : j + 1]);
    }

    out.append(Math.abs(res)).appendNewLine();
  }
