...
public class MinimumEuclideanDistance implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new MinimumEuclideanDistance(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() throws IOException {
    ...
    while (n-- > 0) points[i++] = new int[] {in.nextInt(), in.nextInt()};
    Arrays.sort(
        points, (a, b) -> Long.compare(dist(a, new int[] {0, 0}), dist(b, new int[] {0, 0})));

    long minDis = Long.MAX_VALUE;

    for (int j = 0; j < points.length - 1; j++)
      minDis = Math.min(minDis, dist(points[j], points[j + 1]));

    out.append(minDis).appendNewLine();
  }
