...
auto run() -> void
{
  ...
  int64_t difference[n + 1];
  difference[0] = x[0], difference[n] = 0;
  for (int i = 1; i < n; i++)
    difference[i] = x[i] - x[i - 1];

  auto SUM_SELECT = [](const int64_t &a, const int64_t &b) -> int64_t {
    return a + b;
  };

  SegmentTree<int64_t> tree(difference, n + 1, 0, SUM_SELECT);
  ...
  while (q-- > 0)
  {
    ...
    if (type == 1)
    {
      ...
      tree.set_at_index(a - 1, difference[a - 1]);
      tree.set_at_index(b, difference[b]);
    }
    else
    {
      int k;
      io::cin >> k;
      io::cout << tree.query_range(0, k) << "\n";
    }
  }
}
