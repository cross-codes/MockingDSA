...
auto run() -> void
{
  ...
  auto dfs2 = [&adj, &pos, &count, &values, &sizes,
               &c_values](auto &&dfs2, int u, int p) -> void {
    pos[u] = static_cast<int>(sizes.size());
    sizes.push_back(count[u]);
    c_values.push_back(values[u]);
  ...
  auto SUM_SELECT = [](const int64_t &a, const int64_t &b) -> int64_t {
    return a + b;
  };

  SegmentTree<int64_t> tree(c_values.data(), n, 0L, SUM_SELECT);
  while (q-- > 0)
  {
    int type;
    std::cin >> type;
    ...
    else
    {
      ...
      std::cout << tree.query_range(pos[s - 1], pos[s - 1] + sz) << "\n";
    }
  }
}
