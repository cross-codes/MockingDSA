...
public class SumOfFourValues extends Functions implements Debug {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = i + 1; j < n; j++) {
        ...
        if (twoSum.containsKey(diff)) {
          ...
          out.appendAll(i + 1, " ", j + 1, " ", pair[0], " ", pair[1]).appendNewLine();
          out.flush();
          return;
        }
      }
...
@MultipleInheritanceDisallowed(inheritor = "SumOfFourValues")
