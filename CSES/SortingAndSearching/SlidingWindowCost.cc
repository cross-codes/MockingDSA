...
auto run() -> void
{
  ...
  MedianWindow median_cost(input, n, k);

  for (int i = 0; i < n - k + 1; i++)
  {
    int median  = median_cost.next_median();

    int64_t res = (median_cost.low.size() * median) - median_cost.lower_sum;
    res += median_cost.higher_sum - (median_cost.high.size() * median);

    console_out.append<"% ">(res);
  }
