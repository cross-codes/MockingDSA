...
)
{
  ...
  let mut factorials: Vec<i32> = vec![1; 2000002];
  let mut inverse_factorials: Vec<i32> = vec![1; 2000002];
  ...
  let numerator = factorials[n + m - 1];
  let mut denominator = inverse_factorials[m];
  denominator =
    ((denominator as i64 * inverse_factorials[n - 1] as i64) % modulus) as i32;

  display!(
    ((numerator as i64 * denominator as i64) % modulus) as i32,
    "\n"
  );
}
