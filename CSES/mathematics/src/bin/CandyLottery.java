...
public class CandyLottery extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new CandyLottery(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void consolidateOutput() {
    ...
    for (int m = 1; m <= k; m++) {
      BigDecimal prob = probability(m, k, n);
      expectedValue = expectedValue.add(prob.multiply(BigDecimal.valueOf(m), MC), MC);
    }

    BigDecimal rounded = expectedValue.setScale(6, RoundingMode.HALF_EVEN);
    out.append(rounded.toPlainString()).appendNewLine();
  }

  private BigDecimal probability(int m, int k, int n) {
    BigDecimal bigM = BigDecimal.valueOf(m);
    BigDecimal bigMMinus1 = BigDecimal.valueOf(m - 1);
    BigDecimal bigK = BigDecimal.valueOf(k);

    BigDecimal numerator = bigM.pow(n, MC).subtract(bigMMinus1.pow(n, MC), MC);
    BigDecimal denominator = bigK.pow(n, MC);
...
@MultipleInheritanceDisallowed(inheritor = CandyLottery.class)
