...
auto run() -> void
{
  std::array<int, 1000001> factorials{};
  std::array<int, 1000001> inverse_factorials{};

  factorials[0] = inverse_factorials[0] = 1;
  for (int i = 1; i < 1000001; i++)
  {
    factorials[i]         = (static_cast<int64_t>(i) * factorials[i - 1]) % MOD;
    inverse_factorials[i] = Algebra::mod_pow(factorials[i], MOD - 2, MOD);
  }
  ...
  int64_t numerator   = factorials[n << 1];
  int64_t denominator = (static_cast<int64_t>(inverse_factorials[n + 1]) *
                         inverse_factorials[n]) %
                        MOD;

  io::cout << (numerator * denominator) % MOD << "\n";
}
