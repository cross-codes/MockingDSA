...
public class InversionProbability extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new InversionProbability(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void consolidateOutput() {
    ...
    BigDecimal expectedInversions = BigDecimal.ZERO;
    ...
    for (int b = 0; b < n; b++) {
      for (int a = 0; a < b; a++) {
        ...
        if (r[a] <= r[b])
          ...
          k = ((r[b] * (r[b] - 1)) >> 1) + (r[b] * (r[a] - r[b]));

        BigDecimal numerator = BigDecimal.valueOf(k);
        BigDecimal denominator = BigDecimal.valueOf(r[b]).multiply(BigDecimal.valueOf(r[a]));
        expectedInversions = expectedInversions.add(numerator.divide(denominator, 20, RoundingMode.HALF_EVEN));
      }
    }

    BigDecimal rounded = expectedInversions.setScale(6, RoundingMode.HALF_EVEN);
    out.append(rounded.toPlainString()).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = InversionProbability.class)
