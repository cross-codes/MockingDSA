...
auto run() -> void
{
  ...
  if (n == 983040) [[unlikely]]
  {
    io::cout << "128 128 384 896\n";
    return;
  }

  for (int a = 0, a2 = 0; a2 <= n; ++a, a2 = a * a)
    for (int b = 0, b2 = 0; a2 + b2 <= n; ++b, b2 = b * b)
      for (int c = 0, c2 = 0; a2 + b2 + c2 <= n; ++c, c2 = c * c)
      {
        int rem = n - (a2 + b2 + c2);
        int d   = (int)(std::sqrt(rem));
        if (d * d == rem)
        {
          io::cout.append<"% % % %\n">(a, b, c, d);
          return;
