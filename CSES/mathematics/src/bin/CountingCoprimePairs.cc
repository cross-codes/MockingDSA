...
auto run() -> void
{
  ...
  for (int i = 0; i < n; i++)
  {
    int x;
    io::cin >> x;
    dense_sieve.for_factors(x, false, true, [&factors](const int64_t &factor) {
      factors[factor] += 1;
    });
  }

  int64_t total{(n * (n - 1)) >> 1};
  for (int i = 2; i < 1000001; i++)
  {
    if (factors[i] > 1 && dense_sieve.is_square_free(i))
    {
      int64_t count =
          (static_cast<int64_t>(factors[i]) * (factors[i] - 1)) >> 1;
      total += (dense_sieve.get_parity(i) ? -count : +count);
    }
  }

  io::cout << static_cast<int64_t>(total) << "\n";
}
