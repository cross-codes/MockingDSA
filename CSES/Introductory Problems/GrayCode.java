...
public class GrayCode implements Runnable {
  ...
  public static void main(String[] args) {
    new Thread(null, new GrayCode(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() {
    int n = in.nextInt(), numOfEntries = 1 << n;
    final StringBuilder sb = new StringBuilder(1 << n);

    for (int i = 0; i < numOfEntries; i++) {
      StringBuilder temp = new StringBuilder(n);
      temp.append(Integer.toBinaryString(i ^ (i >> 1))).reverse();
      while (temp.length() < n) temp.append("0");
      temp.reverse().append("\n");
      sb.append(temp);
    }
