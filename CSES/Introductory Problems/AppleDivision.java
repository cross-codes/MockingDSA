...
public class AppleDivision implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new AppleDivision(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() throws IOException {
    int n = in.nextInt();
    long[] weights = in.readLongArray(n);

    long cnt = Long.MAX_VALUE;

    // The 1s represent what we intend to add from each set.
    // A binary sequence from 1 to 2^ n - 1 represents all combinations
    for (int i = 1; i < (1 << n); i++) {
      long sumA = 0L, sumB = 0L;
      for (int j = 0; j < n; j++) {
        if ((i & (1 << j)) != 0) sumA += weights[j];
        else sumB += weights[j];
      }
      cnt = Math.min(cnt, Math.abs(sumA - sumB));
    }
