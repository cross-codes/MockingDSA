...
public class TrailingZeroes implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new TrailingZeroes(), "", 256 * (1L << 20)).start();
  }
  ...
  long exponentOfPInFactorial(long n, int p) {
    long ans = 0L, toAdd = (long) Math.floor((double) n / p);
    int cnt = 1;

    while (toAdd != 0L) {
      ans += toAdd;
      cnt++;
      toAdd = (long) Math.floor((double) n / Math.pow(p, cnt));
    }
  ...
  void solve() throws IOException {
    long n = in.nextLong();
    long powerOf2 = exponentOfPInFactorial(n, 2);
    long powerOf5 = exponentOfPInFactorial(n, 5);

    out.append(Math.min(powerOf2, powerOf5)).appendNewLine();
  }
