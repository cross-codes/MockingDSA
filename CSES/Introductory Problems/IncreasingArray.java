...
public class IncreasingArray implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new IncreasingArray(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() {
    int n = in.nextInt();
    long ans = 0, prev = in.nextLong();

    for (int i = 1; i < n; i++) {
      long currNum = in.nextLong(), diff = currNum - prev;

      if (diff >= 0) prev = currNum;
      else ans += -diff;
    }
