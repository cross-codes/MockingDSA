...
public class NumberSpiral implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new NumberSpiral(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() throws IOException {
    ...
    while (t-- > 0) {
      ...
      long l = Math.max(y, x) - 1;
      if ((l & 1) != 0) {
        if (y < x) out.append(l * l + y).appendNewLine();
        else out.append(l * l + 2 * l - x + 2).appendNewLine();
      } else {
        if (y < x) out.append(l * l + 2 * l - y + 2).appendNewLine();
        else out.append(l * l + x).appendNewLine();
      }
    }
  }
