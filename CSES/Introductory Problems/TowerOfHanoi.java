...
public class TowerOfHanoi implements Runnable {
  ...
  public static void main(String[] args) {
    new Thread(null, new TowerOfHanoi(), "", 256 * (1L << 20)).start();
  }
  ...
  void move(int n, int from, int to, int using, StringBuilder obj) {
    if (n == 0) return;
    move(n - 1, from, using, to, obj);
    obj.append(from).append(" ").append(to).append("\n");
    move(n - 1, using, to, from, obj);
  }

  void solve() throws IOException {
    int n = in.nextInt();
    final StringBuilder sb = new StringBuilder(1 << n);
