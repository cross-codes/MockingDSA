...
public class TwoKnights implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new TwoKnights(), "", 256 * (1L << 20)).start();
  }
   ...
   *
   * <pre>
   * When we have two knights threatening each other, it actually forms a 2 x 3 or
   * 3 x 2 chess board, for each of which, there are 2 ways of placing two knights so that they
   * threaten each other. The number of 2 x 3 and 3 x 2 squares on a chess board of size k x k is (k
   * - 1) * (k - 2) * 2. Hence there are that x 2 ways to place knights so they threaten each other
   * For the final answer, just find the complement
   *
   * </pre>
   */
  void solve() throws IOException {
    int n = in.nextInt();
    for (long i = 2; i <= n; i++)
      out.append((i * i) * (i * i - 1) / 2 - 4 * (i - 1) * (i - 2)).appendNewLine();
  }
