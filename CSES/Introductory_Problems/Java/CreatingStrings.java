...
public class CreatingStrings extends ModuleSignatures implements Debug, Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new CreatingStrings(), "LaunchableDriver", 1048576).start();
  }

  private void consolidateOutput() {
    byte[] s = in.nextLine();
    Arrays.sort(s);
    TreeSet<byte[]> set = new TreeSet<>((a, b) -> Arrays.compare(a, b));

    Array.permute(s, () -> set.add(Arrays.copyOf(s, s.length)));

    out.append(set.size()).appendNewLine();
    set.stream().forEach(elem -> out.append(elem).appendNewLine());
  }
}

@MultipleInheritanceDisallowed(inheritor = "CreatingStrings")
