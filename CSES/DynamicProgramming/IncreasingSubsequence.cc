...
namespace _IncreasingSubsequence
{
...
auto run() -> void
{
  ...
  IndexCompress ic = IndexCompress<int>(std::move(values));
  auto MAX_SELECT  = [&](const int &a, const int &b) { return std::max(a, b); };

  std::size_t maxRange = ic.getMaxRange();
  std::unique_ptr<int[]> empty(new int[maxRange + 1]);
  std::memset(empty.get(), 0x00, sizeof(int) * (maxRange + 1));

  SegmentTree segmentTree =
      SegmentTree<int>(empty, maxRange + 1, 0, MAX_SELECT);

  std::unique_ptr<int[]> d(new int[n]);
  d[0] = 1;
  segmentTree.setAtIndex(ic.getCompressedIndex(a[0]), d[0]);

  for (usize i = 1; i < n; i++)
  {
    d[i] =
        std::max(1, segmentTree.queryRange(0, ic.getCompressedIndex(a[i])) + 1);
    segmentTree.setAtIndex(ic.getCompressedIndex(a[i]), d[i]);
  }
