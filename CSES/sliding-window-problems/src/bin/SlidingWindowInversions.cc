...
namespace _SlidingWindowInversions
{
...
auto run() -> void
{
  ...
  int64_t num_inversions{};
  std::vector<int> x(n), x_copy(n);
  for (int i = 0; i < n; i++)
  {
    io::cin >> x[i];
    x_copy[i] = x[i];
  ...
  IndexCompress generator(std::move(x_copy));
  int N = static_cast<int>(generator.get_max_range());
  BinaryIndexedTree window(N + 1);

  for (int i = 0; i < k; i++)
  {
    int num      = x[i];
    size_t index = generator.get_compressed_index(num);
    int64_t num_higher =
        window.prefix_sum_at(N + 1) - window.prefix_sum_at(index + 1);
    num_inversions += num_higher;
    window.advance(index + 1, 1);
  }

  io::cout << num_inversions << " ";
  int window_start{};

  for (int i = k; i < n; i++)
  {
    int prev_num = x[window_start];
    size_t index = generator.get_compressed_index(prev_num);
    if (index > 0)
    {
      int64_t num_lower = window.prefix_sum_at(index);
      num_inversions -= num_lower;
    }
    ...
    int curr_num = x[i];
    index        = generator.get_compressed_index(curr_num);
    int64_t num_higher =
        window.prefix_sum_at(N + 1) - window.prefix_sum_at(index + 1);
    num_inversions += num_higher;
