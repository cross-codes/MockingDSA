...
auto run() -> void {
  ...
  std::unique_ptr<std::unordered_set<int>[]> array(
      new std::unordered_set<int>[n]);
  for (usize i = 0; i < n; i++) {
    int num;
    std::cin >> num;
    array[i].insert(num);
  ...
  auto UNION_SELECT = [&](const std::unordered_set<int> &a,
  ...
  auto segmentTree = SegmentTree<std::unordered_set<int>>(
      array, n, std::unordered_set<int>(), UNION_SELECT);

  usize a{}, b{};
  while (q-- > 0) {
    std::cin >> a >> b;
    std::cout << segmentTree.queryRange(a - 1, b).size() << "\n";
  }
}
