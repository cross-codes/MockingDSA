...
public class CreatingStringsII implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new CreatingStringsII(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() throws IOException {
    ...
    final int[] factorials = new int[string.length + 1];
    final int[] inverseFactorials = new int[string.length + 1];
    factorials[0] = 1;
    ...
    long res = factorials[string.length];

    for (int i = 0; i < 26; i++) {
      res *= inverseFactorials[freq[i]];
      res %= MOD;
    }
