...
fn run(io: &mut impl IO, _case: u32) {
  let mut iter = io.read_iter::<usize>();

  let (y, x) = (iter.next().unwrap(), iter.next().unwrap());
  let r: usize = y.max(x) - 1;

  let res = match (r & 1 != 0, y < x) {
    (true, true) => r * r + y,
    (true, false) => r * r + 2 * r - x + 2,
    (false, true) => r * r + 2 * r - y + 2,
    (false, false) => r * r + x,
  };

  writeln!(io, "{}", res).unwrap();
}
