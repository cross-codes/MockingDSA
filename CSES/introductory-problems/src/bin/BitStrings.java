...
public class BitStrings extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new BitStrings(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    LongTriFunction modPow = (n, p, m) -> {
      if (m == 1) {
        return 0;
      }

      long result = 1, base = n % m;
      while (p > 0) {
        if ((p & 1) != 0) {
    ...
    long n = in.nextInt();
    out.append(modPow.apply(2, n, 1000000007)).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = BitStrings.class)
