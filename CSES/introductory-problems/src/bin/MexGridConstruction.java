...
public class MexGridConstruction extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new MexGridConstruction(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();
    HashSet<Integer> model = IntStream.rangeClosed(0, 200).boxed().collect(Collectors.toCollection(HashSet::new));

    ArrayList<HashSet<Integer>> rowSets = Stream.generate(() -> new HashSet<>(model))
        .limit(100)
        .collect(Collectors.toCollection(ArrayList::new));

    ArrayList<HashSet<Integer>> colSets = Stream.generate(() -> new HashSet<>(model))
        .limit(100)
        .collect(Collectors.toCollection(ArrayList::new));

    for (int y = 0; y < n; y++) {
      for (int x = 0; x < n; x++) {
        Optional<Integer> mx = rowSets.get(y).stream().filter(colSets.get(x)::contains).findFirst();

        if (mx.isPresent()) {
          rowSets.get(y).remove(mx.get());
          colSets.get(x).remove(mx.get());
          out.append(mx.get()).append(" ");
        } else {
...
@MultipleInheritanceDisallowed(inheritor = MexGridConstruction.class)
