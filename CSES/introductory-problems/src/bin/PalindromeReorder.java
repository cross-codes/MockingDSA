...
public class PalindromeReorder extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new PalindromeReorder(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (byte b : s) {
      freq.compute(b, (k, cv) -> cv == null ? 1 : cv + 1);
    }
    ...
    if ((s.length & 1) != 0) {
      ...
      if (numOdd != 1) {
        ...
        final char rep = (char) odd;
        new String(res, StandardCharsets.US_ASCII).chars().map(c -> (c == '?' ? (char) rep : c)).mapToObj(c -> (char) c)
            .forEach(out::append);
        out.appendNewLine();
      }
    } else {
      ...
      new String(res, StandardCharsets.US_ASCII).chars().mapToObj(c -> (char) c).forEach(out::append);
      out.appendNewLine();
    }
  }
...
@MultipleInheritanceDisallowed(inheritor = PalindromeReorder.class)
