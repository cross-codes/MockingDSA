...
public class TowerOfHanoi extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new TowerOfHanoi(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();

    final IntegerQuadSupplier[] hold = new IntegerQuadSupplier[1];
    ...
    hold[0] = (amt, a, b, c) -> {
      ...
      hold[0].apply(amt - 1, a, c, b);
      hold[0].apply(1, a, b, c);
      hold[0].apply(amt - 1, b, a, c);
    };

    out.append((1 << n) - 1).appendNewLine();
    hold[0].apply(n, 1, 2, 3);
  }

}

@MultipleInheritanceDisallowed(inheritor = TowerOfHanoi.class)
