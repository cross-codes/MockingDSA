...
public class MissingNumber extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new MissingNumber(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    long n = in.nextInt(), sum = 0L;

    for (long i = 0L; i < n - 1; i++) {
      sum += in.nextLong();
    }

    out.append(((n * (n + 1)) >>> 1) - sum).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = MissingNumber.class)
