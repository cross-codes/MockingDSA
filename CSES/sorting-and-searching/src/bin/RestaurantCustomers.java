...
public class RestaurantCustomers extends ModuleSignatures implements Runnable {
  ...
  private void solveCase(int _case) {
    int n = in.nextInt();
    ArrayList<OrderedPair<Integer, Event>> events = new ArrayList<>();

    for (int i = 0; i < n; i++) {
      events.add(new OrderedPair<>(in.nextInt(), Event.ARRIVING));
      events.add(new OrderedPair<>(in.nextInt(), Event.LEAVING));
    }

    events.sort(Comparator.naturalOrder());
    int mx = 0, cnt = 0;
    for (var pair : events) {
      if (pair.second == Event.ARRIVING)
        cnt += 1;
      else
        cnt -= 1;
