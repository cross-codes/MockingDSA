...
public class StickLengths extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new StickLengths(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    Array.sort(p);
    long x = p[n >> 1];
    out.append(Arrays.stream(p).asLongStream().reduce(0L, (long acc, long s) -> acc + Math.abs(s - x))).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = StickLengths.class)
