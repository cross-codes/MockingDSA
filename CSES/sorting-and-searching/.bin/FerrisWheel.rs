...
fn run(io: &mut impl IO, _case: u32) {
  ...
  let (_, x) = (iter.next().unwrap(), iter.next().unwrap());
  let mut p: Vec<i32> = io.read_iter().collect();

  p.sort_unstable();

  let mut cnt: usize = 0;
  let mut fwd = p.iter().enumerate().peekable();
  for (r, w) in p.iter().enumerate().rev() {
    if fwd.peek().is_none() || fwd.peek().unwrap().0 > r {
      break;
    }

    if fwd.peek().unwrap().0 == r {
      cnt += 1;
      fwd.next();
      continue;
    }
