...
fn run(io: &mut impl IO, _case: u32) {
  let mut iter = io.read_iter::<i64>();
  let (n, x) = (iter.next().unwrap(), iter.next().unwrap());
  ...
  if let Some(res) = pos {
    writeln!(io, "{} {}", res.0, res.1).unwrap()
  } else {
    writeln!(io, "IMPOSSIBLE").unwrap()
  }
}
