...
auto run() -> void
{
  ...
  std::string res = s.substr(pos - len[pos] + 1, (len[pos] << 1) - 1);
  for (int i = 1; i < ((len[pos] << 1) - 1); i += 2)
    io::cout << res[i];
