...
namespace _IncreasingSubsequenceII
{
...
public:
  ...
  SegmentTree(std::vector<T> array, T default_value,
              std::function<T(const T &, const T &)> function)
      : offset_(1LL << Algebra::ceil_log2(array.size())),
        tree_(std::make_unique<T[]>(offset_ << 1)), function_(function),
        defaultValue_(default_value)
  {
    std::copy(array.begin(), array.end(), &tree_[offset_]);
...
auto run() -> void
{
  ...
  std::vector<int> x(n, 0), x_cpy(n, 0);
  for (int i = 0; i < n; i++)
  {
    io::cin >> x[i];
    x_cpy[i] = x[i];
  ...
  IndexCompress ic(std::move(x_cpy));

  auto MOD_SUM_SELECT = [](const int &a, const int &b) -> int {
    return (a + b) % MOD;
  };

  auto N = ic.get_max_range();
  int all_zeroes[N + 1];
  std::memset(all_zeroes, 0x00, sizeof(int) * (N + 1));

  SegmentTree<int> tree(all_zeroes, N + 1, 0, MOD_SUM_SELECT);

  // number of increasing sequences ending at x[i]
  int cnt[n];
  for (int i = 0; i < n; i++)
  {
    auto idx = ic.get_compressed_index(x[i]);
    int num  = tree.query_range(0, idx) + 1;
    cnt[i]   = num % MOD;

    int prev = tree.query_range(idx, idx + 1);
    tree.set_at_index(idx, (static_cast<int64_t>(prev) + num) % MOD);
  }
