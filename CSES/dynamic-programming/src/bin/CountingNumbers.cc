...
auto run() -> void
{
  ...
  std::map<std::array<int, 4>, int64_t> lookup{};
  std::string s;

  auto call = [&s, &lookup](auto &&call, int i, int prev, bool equal,
                            bool empty) -> int64_t {
    if (i == static_cast<int>(s.size()))
      return 1L;

    std::array<int, 4> params = {i, prev, equal, empty};
    ...
    for (int d = 0; d < 10; d++)
    {
      ...
      res +=
          call(call, i + 1, d, equal && (d == s[i] - '0'), empty && (d == 0));
    }

    return lookup[params] = res;
  };

  auto count = [&call, &s, &lookup](int64_t x) -> int64_t {
    s = std::to_string(x);
    lookup.clear();
    return call(call, 0, -1, true, true);
  };

  io::cout << count(b) - count(a - 1) << "\n";
};
} // namespace _CountingNumbers
