...
public class MovieFestival implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new MovieFestival(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() throws IOException {
    int n = in.nextInt();
    PriorityQueue<int[]> intervals = new PriorityQueue<>((a, b) -> Integer.compare(a[1], b[1]));
    for (int i = 0; i < n; i++)
      intervals.add(new int[] { in.nextInt(), in.nextInt() });

    int latestEnd = intervals.poll()[1], cnt = 1;

    while (!intervals.isEmpty()) {
      int[] interval = intervals.poll();
      int currStart = interval[0];
      int currEnd = interval[1];
      if (currStart >= latestEnd) {
        cnt++;
        latestEnd = currEnd;
      }
    }
  ...
  static class OutputWriter {
    ...
    public OutputWriter append(String s) throws IOException {
      ...
      for (int i = 0; i < length; i++)
        this.buffer[pos++] = (byte) s.charAt(i);
      return this;
    }
