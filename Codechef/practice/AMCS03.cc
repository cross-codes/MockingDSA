...
auto p(double t, int s[], int d[], int i) -> double
{
  return s[i] * t + d[i];
}

auto f(double t, int s[], int d[], int n) -> double
{
  double min_p = __builtin_huge_valf64();
  double max_p = 0;
  for (int i = 0; i < n; i++)
  {
    min_p = std::min(min_p, p(t, s, d, i));
    max_p = std::max(max_p, p(t, s, d, i));
  }

  return max_p - min_p;
}

auto unimodal_min(double l, double r, int s[], int d[], int n)
{
  double ε = 1e-9;
  while (r - l > ε)
  {
...
auto run() -> void
{
  ...
  out << std::fixed << std::setprecision(6) << unimodal_min(0, k, s, d, n);
