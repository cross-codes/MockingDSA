...
auto run() -> void {
  ...
  for (int i = 0, a, b; i < m; i++) {
    ...
    auto fwd_rev = std::make_shared<Edge>(b, a, 1, EdgeType::FORWARD);
    auto bck_rev = std::make_shared<Edge>(a, b, 0, EdgeType::BACKWARD);
    fwd_rev->rev = bck_rev;
    bck_rev->rev = fwd_rev;
    res_adj[a].push_back(bck_rev);
    res_adj[b].push_back(fwd_rev);
    edges.push_back(fwd_rev);
  }
  ...
  std::vector<bool> belongs_to_A(n, false);
  auto dfs = [&belongs_to_A, &res_adj](auto&& dfs, int u) -> void {
    belongs_to_A[u] = true;
    for (const auto& e : res_adj[u]) {
      if (e->residual_capacity() > 0 && !belongs_to_A[e->to]) {
        dfs(dfs, e->to);
      }
    }
  };
  ...
  for (const auto& e : edges) {
    int u{e->from}, v{e->to};
    if (belongs_to_A[u] && !belongs_to_A[v]) {
      std::cout << u + 1 << " " << v + 1 << "\n";
    }
  }
}
