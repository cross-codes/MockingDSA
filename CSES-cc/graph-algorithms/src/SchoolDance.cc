...
auto run() -> void {
  ...
  std::vector<std::vector<std::shared_ptr<Edge>>> res_adj(n + m + 2);
  std::vector<std::shared_ptr<Edge>> edges{};
  for (int i = 0, a, b; i < k; i++) {
    std::cin >> a >> b;
    b += n;

    auto fwd = std::make_shared<Edge>(a, b, 1, EdgeType::FORWARD);
  ...
  for (int a = 1; a <= n; a++) {
    auto src_fwd = std::make_shared<Edge>(0, a, 1, EdgeType::FORWARD);
    auto src_bck = std::make_shared<Edge>(a, 0, 0, EdgeType::BACKWARD);

    src_fwd->rev = src_bck;
    src_bck->rev = src_fwd;
    res_adj[0].push_back(src_fwd);
    res_adj[a].push_back(src_bck);
  }

  for (int b = n + 1; b <= n + m; b++) {
    auto sink_fwd = std::make_shared<Edge>(b, n + m + 1, 1, EdgeType::FORWARD);
    auto sink_bck = std::make_shared<Edge>(n + m + 1, b, 0, EdgeType::BACKWARD);
    sink_fwd->rev = sink_bck;
    sink_bck->rev = sink_fwd;
    res_adj[b].push_back(sink_fwd);
    res_adj[n + m + 1].push_back(sink_bck);
  }

  std::vector<std::shared_ptr<Edge>> edge_to(n + m + 2);
  std::vector<int> visited(n + m + 2);
  int iteration{};
  ResettableQueue<int> queue(n + m + 2);

  auto bottleneck = [&]() -> int64_t {
    int64_t mn{INT64_MAX};
    for (int v = n + m + 1; v != 0; v = edge_to[v]->from) {
