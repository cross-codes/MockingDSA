...
auto run() -> void {
  ...
  std::array<int, 1000001 << 1> factorial{}, inverse_factorial{};
  ...
  int res =
      (((static_cast<int64_t>(factorial[n + m - 1]) * inverse_factorial[m]) %
        MOD) *
       static_cast<int64_t>(inverse_factorial[n - 1])) %
      MOD;
