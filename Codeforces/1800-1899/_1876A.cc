...
int main() {
  ...
  while (t-- > 0) {
    ...
    constexpr auto cmp = [](std::pair<ll, ll> a, std::pair<ll, ll> b) -> bool {
      return a.second < b.second;
    };

    std::sort(pairs.begin(), pairs.end(), cmp);

    ll bestPrice = n * p, cnt = 0LL;
    for (int i = 0; i < n; i++) {
      auto pair = pairs[i];
      ll shareable = pair.first, cost = pair.second;
      if (cost >= p || cnt >= n - 1)
        break;
      ll toShare = std::min(n - cnt - 1, shareable);
      bestPrice -= (p - cost) * toShare;
      cnt += toShare;
    }
