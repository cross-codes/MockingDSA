...
public class _1831B implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new _1831B(), "", 256 * (1L << 20)).start();
  }
  ...
  int[] genSequences(int[] arr, int n) {
    int[] ans = new int[2 * n + 1];
    int st = 0;
    for (int i = 1; i < n; i++) {
      if (arr[i] != arr[i - 1]) {
        ans[arr[i - 1]] = Math.max(ans[arr[i - 1]], i - st);
        st = i;
      }
    }
    ans[arr[n - 1]] = Math.max(ans[arr[n - 1]], n - st);
    return ans;
  }
  ...
  void solve() {
    ...
    while (t-- > 0) {
      int n = in.nextInt();
      int[] aseq = genSequences(in.readIntegerArray(n), n);
      int[] bseq = genSequences(in.readIntegerArray(n), n);
      int res = 0;
      for (int i = 0; i <= 2 * n; i++) {
        res = Math.max(res, aseq[i] + bseq[i]);
      }
