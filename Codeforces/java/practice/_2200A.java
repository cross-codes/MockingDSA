...
public class _2200A extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2200A(), "LaunchableDriver", 67108864L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    IntToIntFunction round = (s) -> {
      int[] dishes = a.clone(), target = new int[n + 1];
      int last = s, curr = s;
      while (!Arrays.equals(dishes, target)) {
        if (dishes[curr] > 0) {
          dishes[curr] -= 1;
          last = curr;
        }

        curr = Math.floorMod(curr, n) + 1;
      }
    ...
    for (int s = 1; s <= n; s++) {
      canWin[round.run(s)] = true;
    }

    int cnt = 0;
    for (boolean b : canWin) {
      if (b) {
        cnt += 1;
      }
    }
...
@MultipleInheritanceDisallowed(inheritor = _2200A.class)
