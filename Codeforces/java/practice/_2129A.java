...
public class _2129A extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2129A(), "LaunchableDriver", 67108864L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int i = 0; i < n; i++) {
      pairs[i] = new IntPair(in.nextInt(), in.nextInt());
    }

    DisjointSetForest dsu = new DisjointSetForest(n << 1 | 1);
    IntegerArrayList ial = new IntegerArrayList();
    for (int i = 0; i < n; i++) {
      var entry = pairs[i];
      boolean united = dsu.unite(entry.a(), entry.b());
      if (united) {
        ial.add(i + 1);
      }
    }

    out.append(ial.size()).appendNewLine();
    for (int e : ial) {
      out.append(e).append(" ");
    }
...
@MultipleInheritanceDisallowed(inheritor = _2129A.class)
