...
public class _668948E extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _668948E(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt(), x = in.nextInt();
    int[] d = in.readIntegerArray(n);
    ...
    for (int c : d) {
      ...
      for (int j = c; j <= x; j++) {
        if (prev[j - c] != -1) {
          mx[j] = Math.max(mx[j], prev[j - c] + 1);
        }
        if (mx[j - c] != -1) {
          mx[j] = Math.max(mx[j], mx[j - c]);
        }
      }
    }

    out.append(mx[x]).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = _668948E.class)
