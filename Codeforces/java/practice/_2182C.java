...
public class _2182C extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2182C(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();
    int[] a = in.readIntegerArray(n), b = in.readIntegerArray(n), c = in.readIntegerArray(n);

    IntPredicate loopI = (pos) -> {
      for (int i = 0; i < n; i++) {
        if (a[Math.floorMod(i + pos, n)] >= b[i]) {
          return false;
        }
      }
    ...
    IntPredicate loopK = (pos) -> {
      for (int i = 0; i < n; i++) {
        if (c[Math.floorMod(i + pos, n)] <= b[i]) {
          return false;
        }
      }
    ...
    out.append(numI * numK * n).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = _2182C.class)
