...
public class _2197B extends DerivativesCore implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2197B(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();

    int[] p = new int[n], pos = new int[n + 1];
    for (int i = 0; i < n; i++) {
      p[i] = in.nextInt();
      pos[p[i]] = i;
    }
...
@MultipleInheritanceDisallowed(inheritor = _2197B.class)
abstract class DerivativesCore {
  @FunctionalInterface
  interface IntegerBiFunction {
    int apply(int target, int pos);
  }
