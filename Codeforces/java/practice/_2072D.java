...
public class _2072D extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2072D(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    MergeSortTree mst = new MergeSortTree(0, n, a);

    int[] inversions = new int[1];
    for (int i = 0; i < n; i++) {
      inversions[0] += mst.count(i + 1, n, 1, a[i] - 1);
    }

    IntegerTriFunction invXchg = (l, r) -> {
      int inv = inversions[0];
    ...
    int best = inversions[0];
    int lB = -1, rB = -1;
    for (int l = 0; l < n; l++) {
      for (int r = l + 1, x; r < n; r++) {
        if ((x = invXchg.apply(l, r)) < best) {
          best = x;
          lB = l + 1;
          rB = r + 1;
        }
    ...
    if (lB == -1) {
      out.append("1 1").appendNewLine();
    } else {
      out.append(lB).append(" ").append(rB).appendNewLine();
    }
  }

}

@MultipleInheritanceDisallowed(inheritor = _2072D.class)
abstract class ModuleSignatures {
  @FunctionalInterface
  interface IntegerTriFunction {
...
class MergeSortTree {
  ...
  int count(int from, int excludeTo, int inclusiveRangeStart, int inclusiveRangeEnd) {
