...
public class _2189B extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2189B(), "LaunchableDriver", 67108864L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    ArrayList<LngT> jumps = new ArrayList<>(n);
    for (int i = 0; i < n; i++) {
      jumps.add(new LngT(in.nextLong(), in.nextLong(), in.nextLong()));
    }

    Comparator<LngT> cmp = Comparator.comparingLong(jump -> jump.a() * jump.b() - jump.c());
    jumps.sort(cmp);

    long d = 0;
    for (int i = 0; i < n; i++) {
      LngT jump = jumps.get(i);
      d += (jump.b() - 1) * jump.a();
    }
    ...
    LngT last = jumps.getLast();
    if (last.a() * last.b() <= last.c()) {
      out.append(-1).appendNewLine();
    } else {
      BigInteger dist = BigInteger.valueOf(d - last.c() + last.a());

      BigInteger A = BigInteger.valueOf(last.a()), B = BigInteger.valueOf(last.b());
      BigInteger C = BigInteger.valueOf(last.c()), X = BigInteger.valueOf(x);

      LongPredicate pred = (long rollbacks) -> {
        BigInteger R = BigInteger.valueOf(rollbacks);
        BigInteger firstTerm = dist.add(R.multiply(B.multiply(A).subtract(C))),
            secondTerm = B.subtract(BigInteger.ONE).multiply(A);
        return firstTerm.add(secondTerm).compareTo(X) >= 0;
      };
      ...
      out.append(1 + R).appendNewLine();
    }
  }

}

@MultipleInheritanceDisallowed(inheritor = _2189B.class)
abstract class ModuleSignatures {
  public record LngT(long a, long b, long c) {
  };
}
