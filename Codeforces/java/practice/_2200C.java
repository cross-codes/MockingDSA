...
public class _2200C extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2200C(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int j = i; j < n; j++) {
      if (s[j] != '*') {
        if (!stack.isEmpty() && stack.peekLast() == s[j]) {
          stack.removeLast();
        } else {
          stack.addLast(s[j]);
        }
    ...
    out.append(stack.isEmpty() ? "YES" : "NO").appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = _2200C.class)
