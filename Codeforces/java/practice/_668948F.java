...
public class _668948F extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _668948F(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();
    int[] f = in.readIntegerArray(n);
    int q = in.nextInt();

    int[][] succ = new int[n][31];
    for (int i = 0; i < n; i++) {
      succ[i][0] = i;
      succ[i][1] = f[i] - 1;
    }
    ...
    IntBinaryOperator successor = (x, k) -> {
      int curr = x;
      while (k != 0) {
        int pos = Integer.numberOfTrailingZeros(k);
        curr = succ[curr][pos + 1];
        k = k & (k - 1);
      }
    ...
    int ulim = Integer.highestOneBit(n) << 1;
    for (int i = 0; i < q; i++) {
      int u = in.nextInt() - 1, v = in.nextInt() - 1;
      if (successor.applyAsInt(u, ulim) == successor.applyAsInt(v, ulim)) {
...
@MultipleInheritanceDisallowed(inheritor = _668948F.class)
