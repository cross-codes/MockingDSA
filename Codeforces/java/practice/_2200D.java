...
public class _2200D extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2200D(), "LaunchableDriver", 67108864L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    ArrayList<Integer> M_star = new ArrayList<>();
    for (int i = 0; i < M.size(); i++) {
      M_star.add(M.get((minIdx + i) % M.size()));
    }
    ...
    for (int i = 0; i < S.size(); i++) {
      if (M_star.get(0) < S.get(i)) {
        for (int j = 0; j < i; j++) {
          out.append(S.get(j)).append(" ");
        }
        for (int e : M_star) {
          out.append(e).append(" ");
        }
...
@MultipleInheritanceDisallowed(inheritor = _2200D.class)
