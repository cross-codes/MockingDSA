...
public class _2132C2 extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2132C2(), "LaunchableDriver", 67108864L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    int[] deals = new int[21];
    int mn = 0, curr = n;
    while (curr != 0) {
      int x = (int) Math.floor(Math.log(curr) / Math.log(3) + 1e-9);
      deals[x] += 1;
      curr -= (int) Math.pow(3, x);
      mn += 1;
    }
    ...
    if (k < mn) {
      ...
      while (idx >= 1) {
        ...
        int amt = deals[idx];
        int mmax = (k - cnt) >> 1;
        cnt += 2 * Math.min(mmax, amt);
        deals[idx] -= Math.min(mmax, amt);
        deals[idx - 1] += 3 * Math.min(mmax, amt);
        idx -= 1;
      }

      long total = 0;
      for (int i = 0; i < 20; i++) {
        total += cost[i] * deals[i];
      }

      out.append(total).appendNewLine();
    }
  }

}

@MultipleInheritanceDisallowed(inheritor = _2132C2.class)
abstract class ModuleSignatures {
  protected long[] pow3 = new long[21], cost = new long[20];

  abstract protected void precompute();
}
