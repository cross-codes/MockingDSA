...
public class _668948C extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _668948C(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    IntegerArrayList primeFactors = new IntegerArrayList();

    IntConsumer generatePrimeFactors = (n) -> {
      int x = 2;
      while (x * x <= n) {
        while ((n % x) == 0) {
          primeFactors.add(x);
    ...
    generatePrimeFactors.accept((in.nextInt()));
    out.append(primeFactors.get(0)).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = _668948C.class)
