...
public class _2200B extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2200B(), "LaunchableDriver", 67108864L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    Predicate<ArrayList<Integer>> isNonDecreasing = (arr) -> {
    ...
    DFS dfs = (instance, positions) -> {
      if (memo.containsKey(positions)) {
        return memo.get(positions);
      }
      ...
      if (isNonDecreasing.test(newArr)) {
        memo.put(positions, newArr.size());
        return newArr.size();
      }

      int minVal = newArr.size();
      for (int i = 0; i < n; i++) {
        if ((positions & (1 << i)) != 0) {
          int nextPositions = positions ^ (1 << i);
          minVal = Math.min(minVal, instance.run(instance, nextPositions));
        }
      }

      memo.put(positions, minVal);
      return memo.get(positions);
    };
    ...
    out.append(dfs.run(dfs, positions)).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = _2200B.class)
abstract class ModuleSignatures {
  @FunctionalInterface
  interface DFS {
    int run(DFS instance, int mask);
  }
}
