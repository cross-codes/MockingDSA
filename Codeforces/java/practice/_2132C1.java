...
public class _2132C1 extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2132C1(), "LaunchableDriver", 67108864L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    long total = 0;
    while (n != 0) {
      int x = (int) Math.floor(Math.log(n) / Math.log(3) + 1e-9);
      total += (long) (Math.pow(3, x + 1) + x * Math.pow(3, x - 1));
      n -= (int) Math.pow(3, x);
    }
...
@MultipleInheritanceDisallowed(inheritor = _2132C1.class)
