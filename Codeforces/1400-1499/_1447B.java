...
public class _1447B implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new _1447B(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() throws IOException {
    ...
    while (t-- > 0) {
      ...
      long absoluteSum = 0L;
      int numNegative = 0, leastAbsolute = Integer.MAX_VALUE;
      ...
      for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
          ...
          leastAbsolute = Math.min(leastAbsolute, Math.abs(num));
        }
      }

      if ((numNegative & 1) == 0) out.append(absoluteSum).appendNewLine();
      else out.append(absoluteSum - (leastAbsolute << 1)).appendNewLine();
    }
  }
