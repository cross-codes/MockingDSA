...
@Launchable(author = "cross-codes", hostname = "inspiron", judge = "Codeforces")
public class _2051D extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2051D(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void consolidateOutput() {
    ...
    while (t-- > 0) {
      ...
      for (int i = 0; i < n; i++) {
        long currDiff = sum - nums[i];
        long lowerBound = currDiff - y, upperBound = currDiff - x;

        int lowerIndex = Arrays.binarySearch(nums, i + 1, n, lowerBound);
        if (lowerIndex < 0) {
        ...
        int upperIndex = Arrays.binarySearch(nums, i + 1, n, upperBound);
        if (upperIndex < 0) {
      ...
      out.append(numberOfPairs).appendNewLine();
    }
  }

}

@MultipleInheritanceDisallowed(inheritor = _2051D.class)
