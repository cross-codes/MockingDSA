...
public class _2037C extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2037C(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void consolidateOutput() {
    ...
    while (t-- > 0) {
      ...
      if ((n & 1) != 0) {
        long bridge = n;
        if (Algebra.isPrime(bridge + 2L)) {
          for (long i = n - 1L; i > 2L; i -= 2L) {
            if (!Algebra.isPrime(bridge + i)) {
              for (long j = 1L; j <= n; j += 2L)
                out.append(j).append(" ");
              out.append(i).append(" ");
              for (long j = 2L; j <= n - 1L; j += 2L) {
                if (j == i)
                  continue;
                out.append(j).append(" ");
              }
          ...
          for (long i = 1L; i <= n; i += 2L)
            out.append(i).append(" ");
          for (long i = 2; i <= n - 1L; i += 2L)
            out.append(i).append(i == n - 1L ? "" : " ");
          found = true;
        }
      } else {
        long bridge = n - 1L;
        if (Algebra.isPrime(bridge + 2L)) {
          for (long i = n; i > 2L; i -= 2L) {
            if (!Algebra.isPrime(bridge + i)) {
              for (long j = 1L; j <= n - 1L; j += 2L)
                out.append(j).append(" ");
              out.append(i).append(" ");
              for (long j = 2L; j <= n; j += 2L) {
                if (j == i)
                  continue;
                out.append(j).append(" ");
              }
...
class Debug {
  ...
  public final void print(Object... __VA_ARGS__) {
    if (this.local) {
