...
public class _2051C extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2051C(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void consolidateOutput() {
    ...
    while (t-- > 0) {
      ...
      int[] a = in.readIntegerArray(m);
      HashSet<Integer> knownQuestions = new HashSet<>();
      for (int i = 0; i < k; i++) {
        knownQuestions.add(in.nextInt());
      }

      for (int removedQuestion : a) {
        if (k <= n - 2) {
          out.append(0);
          continue;
        }
        if (k == n - 1) {
          if (knownQuestions.contains(removedQuestion)) {
            out.append(0);
            continue;
          } else {
            out.append(1);
...
@MultipleInheritanceDisallowed(inheritor = _2051C.class)
