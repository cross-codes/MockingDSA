...
public class _2009B implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new _2009B(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() throws IOException {
    ...
    while (t-- > 0) {
      int n = in.nextInt();
      StringBuilder tt = new StringBuilder(10);
      while (n-- > 0) {
        byte[] map = in.next();
        for (int i = 1; i < 5; i++) if ((char) map[i - 1] == '#') tt.append(i).append(" ");
      }
      tt.deleteCharAt(tt.length() - 1).reverse();
      out.append(tt.toString()).appendNewLine();
    }
  }
