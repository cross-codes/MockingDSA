...
int main() {
  ...
  while (t-- > 0) {
    ...
    __gnu_pbds::gp_hash_table<int, std::nullptr_t> known_questions{};
    for (int i = 0; i < k; i++) {
      int num;
      cin >> num;
      known_questions.insert({num, nullptr});
    }

    for (int removed_question : a) {
      if (k <= n - 2)
        cout << 0;
      else if (k == n - 1) {
        if (known_questions.find(removed_question) != known_questions.end())
          cout << 0;
        else
          cout << 1;
      } else
