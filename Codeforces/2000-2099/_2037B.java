...
public class _2037B extends ModuleSignatures implements Debug, Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new _2037B(), "LaunchableDriver", 1048576).start();
  }
  ...
  private void consolidateOutput() {
    ...
    while (t-- > 0) {
      int k = in.nextInt();
      int[] array = in.readIntegerArray(k);
      ...
      for (int i = 0; i < k; i++) {
        if (product % array[i] == 0) {
          int pos = Arrays.binarySearch(array, product / array[i]);
          if (pos >= 0) {
            if (pos == i) {
              if ((i > 0 && array[pos - 1] == array[pos])
                  || (i < n - 1 && array[pos + 1] == array[pos])) {
                n = array[i];
                m = array[pos];
                break;
              }
      ...
      out.appendAll(n, " ", m).appendNewLine();
    }
  }

}

@MultipleInheritanceDisallowed(inheritor = "_2037B")
