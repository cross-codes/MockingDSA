...
public:
  ...
  inline static int log2(std::int64_t n) { return 63 - __builtin_clzll(n); }
  inline static int ceilLog2(std::int64_t n) { return 64 - __builtin_clzll(n); }
...
auto run() -> void {
  ...
  while (true) {
    n++;
    if (Algebra::isPrime(n)) {
      std::println("{}", (n == m) ? "YES" : "NO");
      return;
    }
  }
}
