...
auto total_water(int64_t h, int64_t a[], int n) -> int64_t
{
  int64_t water{};
  for (int i = 0; i < n; i++)
    water += std::max(h - a[i], static_cast<int64_t>(0));
...
auto run() -> void
{
  ...
  int64_t low{min_height}, high{x + max_height}, mid;
  while (low <= high)
  {
    mid = (low + high) >> 1;
    if (total_water(mid, a, n) <= x)
      low = mid + 1;
    else if (total_water(mid, a, n) > x)
      high = mid - 1;
  }

  console_out << ((total_water(low, a, n) <= x) ? low : low - 1) << "\n";
}
