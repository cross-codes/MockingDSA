...
{
private:
  constexpr inline static int N = 300000;
...
auto run() -> void
{
  ...
  int64_t max_sum{};
  dense_sieve.for_factors(n, true, false, [&max_sum, &n, &prefix](int k) {
    int64_t max_range_query{INT64_MIN}, min_range_query{INT64_MAX};
    for (int i = 0; i <= n - k; i += k)
    {
      int64_t range_query = prefix[i + k] - prefix[i];
      max_range_query     = std::max(max_range_query, range_query);
      min_range_query     = std::min(min_range_query, range_query);
    }

    max_sum = std::max(max_sum, max_range_query - min_range_query);
  });
