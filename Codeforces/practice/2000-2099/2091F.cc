...
std::array<std::string, 2000> s;
std::array<std::array<std::array<int, 2>, 2000>, 2000> t{};
std::array<std::array<std::array<int_fast64_t, 2>, 2000>, 2000> prefix{};

auto get_sum(int y, int x_low, int y_low, int f) -> int_fast64_t
{
  int_fast64_t res{prefix[y][y_low][f]};

  if (x_low != 0)
    res -= prefix[y][x_low - 1][f];

  return res;
}

auto get(int y, int x, int f, const int &n, const int &m, const int &d) -> int
{
  if (s[y][x] != 'X')
    return 0;
  ...
  if (f == 0)
  {
    res += get_sum(y, std::max(0, x - d), std::min(m - 1, x + d), 1);
    res -= t[y][x][1];
  }
  ...
  {
    res +=
        get_sum(y + 1, std::max(0, x - d + 1), std::min(m - 1, x + d - 1), 0);
  }
...
auto run() -> void
{
  ...
  for (int y = n - 1; y >= 0; y--)
  {
    ...
    {
      for (int x = 0; x < m; x++)
        prefix[y][x][f] = t[y][x][f] = get(y, x, f, n, m, d);

      for (int x = 1; x < m; x++)
        prefix[y][x][f] += prefix[y][x - 1][f];
    }
  }
