...
public class _1659A implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new _1659A(), "", 256 * (1L << 20)).start();
  }
  ...
  int[] findLargestRepeatingSubsequence(String s, char c) {
    int n = s.length();
    int[] res = new int[2];
    int maxLen = 0;
    int currentStart = -1;
    ...
    for (int i = 0; i < n; i++) {
      if (s.charAt(i) == c) {
        ...
        if (currentLen > maxLen) {
          maxLen = currentLen;
          res[0] = currentStart;
          res[1] = currentStart + currentLen - 1;
        }
        currentStart = -1;
        currentLen = 0;
      }
    ...
    if (maxLen == 0 && currentLen == 0) return new int[] {-1, -1};
  ...
  void solve() {
    ...
    while (t-- > 0) {
      ...
      int p = r % (b + 1);
      final StringBuilder tt = new StringBuilder(n), y = new StringBuilder(n);
      for (int i = 0; i < Math.ceil(r / (b + 1)); i++) y.append("R");
      for (int i = 0; i < b + 1; i++) {
        if (i > 0) tt.append("B");
        tt.append(y);
        if (p-- > 0) tt.append("R");
