...
public class _1690D implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new _1690D(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() throws IOException {
    ...
    while (t-- > 0) {
      ...
      for (int i = l; i <= r; i++) {
        if ((char) seq[i] == 'W') white++;
      }

      boolean startIsWhite = (char) seq[l] == 'W';
      min = Math.min(min, white);
      l++;
      r++;

      while (r < n) {
        if (startIsWhite) white--;
        white += (char) seq[r] == 'W' ? 1 : 0;
        startIsWhite = seq[l] == 'W';
        min = Math.min(min, white);
        l++;
        r++;
      }

      out.append(min).appendNewLine();
    }
  }
