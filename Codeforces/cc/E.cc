...
auto run() -> void
{
  ...
  int freq[n + 1];
  std::memset(freq, 0x00, sizeof(int) * (n + 1));
  ...
  // to make e <= mex the mex, remove f0 - 1, f1 - 1 ... fe+1, fe+2..
  int difference[n + 2];
  std::memset(difference, 0x00, sizeof(int) * (n + 2));
  for (int e = 0; e <= mex; e++)
  {
    int min = freq[e];
    int max = prefix[e] - e + (prefix[n + 1] - prefix[e]);
  ...
  new_prefix[0] = 0;
  for (int i = 1; i <= n + 2; i++)
    new_prefix[i] = new_prefix[i - 1] + difference[i - 1];

  for (int i = 0; i <= n; i++)
    io::cout << new_prefix[i + 1] << " ";
