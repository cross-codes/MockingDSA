...
void add_magic_powder(int k, std::multiset<std::tuple<int, int, int>> &r)
{
  while (k != 0)
  {
    auto min_tuple = r.begin();
    int to_add     = std::get<1>(*min_tuple);
    if (k < to_add)
      break;
    else
    {
      auto new_tuple =
          std::make_tuple(std::get<0>(*min_tuple) + 1, std::get<2>(*min_tuple),
                          std::get<2>(*min_tuple));
      k -= to_add;
      r.erase(min_tuple);
      r.insert(new_tuple);
...
auto run() -> void
{
  ...
  int a[n];
  std::multiset<std::tuple<int, int, int>> r; // fac, rem, a[i]
  for (int i = 0; i < n; i++)
    io::cin >> a[i];
  ...
  for (int i = 0; i < n; i++)
  {
    int b;
    io::cin >> b;
    r.insert(std::make_tuple(b / a[i], a[i] - (b % a[i]), a[i]));
  }

  add_magic_powder(k, r);

  io::cout << std::get<0>(*r.begin()) << "\n";
}
