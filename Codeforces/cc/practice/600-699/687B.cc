...
auto run() -> void
{
  ...
  auto update_map = [&primes](int prime, int power) {
  ...
  for (int i = 0; i < n; i++)
  {
    int x;
    console_in >> x;
    dense_sieve.for_prime_factors_and_powers(x, update_map);
  }

  bool valid{true};

  auto check = [&primes, &valid](int prime, int power) {
    int max_power = primes[prime];
    if (max_power < power)
      valid = false;
  };

  dense_sieve.for_prime_factors_and_powers(k, check);

  console_out << (valid ? "Yes\n" : "No\n");
}
