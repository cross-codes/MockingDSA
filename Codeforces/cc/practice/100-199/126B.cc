...
auto run() -> void
{
  ...
  constexpr int B = static_cast<int>(1e9 - 7);
  ...
  for (int k = n - 2; k >= 1; k--)
  {
    int prefix = h.get_hash(0, k);
    int suffix = h.get_hash(n - k, n);

    if (prefix == suffix)
    {
      for (int i = 1; i < n - k; i++)
      {
        int middle = h.get_hash(i, i + k);
        if (middle == prefix)
        {
          io::cout << s.substr(i, k) << "\n";
          return;
        }
      }
    }
