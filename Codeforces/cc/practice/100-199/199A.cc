...
struct Algebra {
 ...
 private:
  ...
  inline static std::uint64_t MOD = static_cast<std::uint64_t>(1e9 + 7);
 ...
 public:
  ...
  inline static std::uint64_t fibonnaciNumber(std::uint64_t n,
    ...
    while (n > 0) {
      if (!(n & 1)) {
        std::uint64_t qq = (q * q) % modulus;
        q                = ((2 * p * q) % modulus + qq) % modulus;
        p                = ((p * p) % modulus + qq) % modulus;
        n /= 2;
      } else {
        std::uint64_t aq = (a * q) % modulus;
...
auto run() -> void {
  ...
  if (n == 0) [[unlikely]] {
    ...
    u64 low = 1ULL, high = 44ULL, mid;
    while (low <= high) {
      mid     = (low + high) >> 1;
      u64 fib = Algebra::fibonnaciNumber(mid);
      if (fib > n)
        high = mid - 1;
      else if (fib < n)
        low = mid + 1;
    ...
    std::println("{} {} {}", Algebra::fibonnaciNumber(mid - 1),
                 Algebra::fibonnaciNumber(mid - 2), 0);
  }
}
