...
auto get_parity(int x) -> Parity
{
  return (x & 1) ? Parity::ODD : Parity::EVEN;
}
...
auto run() -> void
{
  ...
  Parity min_parity = get_parity(a[0]);
  Parity max_parity = get_parity(a[n - 1]);

  int num_steps{n - 1};
  for (int i = 0; i < n - 1; i++)
  {
    Parity test = get_parity(a[i]);
    if (test == max_parity)
      num_steps = std::min(num_steps, i);
  }
  ...
  for (int i = n - 1; i > 0; i--)
  {
    Parity test = get_parity(a[i]);
    if (test == min_parity)
      num_steps = std::min(num_steps, n - 1 - i);
  }
