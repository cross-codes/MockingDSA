...
auto run() -> void
{
  ...
  int mx = *std::max_element(x, x + n);
  int mn = *std::min_element(x, x + n);

  if (mx == mn)
  {
    io::cout << std::abs(mx - s) << "\n";
    return;
  }

  int minr = std::abs(mx - s);
  int minl = std::abs(mn - s);

  if ((mx > s && mn > s) || (mx < s && mn < s))
  {
    io::cout << std::max(minr, minl) << "\n";
    return;
  }

  io::cout << 2 * std::min(minr, minl) + std::max(minr, minl) << "\n";
}
