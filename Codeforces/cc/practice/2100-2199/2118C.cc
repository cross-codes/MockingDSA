...
auto run() -> void
{
  ...
  std::vector<std::vector<uint64_t>> cost(
      n, std::vector<uint64_t>(64, UINT64_MAX));
  auto set_costs = [&cost, &a, &n]() -> void {
    for (int i = 0; i < n; i++)
    {
      uint64_t x = a[i];
      int op     = 0;
      ...
      while (x != UINT64_MAX)
      {
        uint64_t pos = __builtin_ffsll(~x) - 1;
        cost[i][op]  = (x | (1ULL << pos)) - x;
        op += 1;
        x |= (1ULL << pos);
      }
    }
  ...
  auto min_cost = [&cost, &a,
                   &n](int idxs[]) -> std::tuple<uint64_t, int, uint64_t> {
    uint64_t min_diff{UINT64_MAX}, new_value{};
    int pos{};
    for (int i = 0; i < n; i++)
    {
      uint64_t diff = cost[i][idxs[i]];
      if (diff < min_diff)
      {
        min_diff   = diff;
        pos        = i;
    ...
    return std::make_tuple(min_diff, pos, new_value);
  };

  int idxs[n];
  std::memset(idxs, 0x00, sizeof(int) * n);

  set_costs();
  while (k > 0)
  {
    auto [diff, idx, updated_value] = min_cost(idxs);
    if (k >= diff)
    {
      k -= diff;
      a[idx] = updated_value;
  ...
  uint64_t num_set{};
  for (uint64_t e : a)
    num_set += __builtin_popcountll(e);
