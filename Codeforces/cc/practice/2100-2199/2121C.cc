...
auto run() -> void
{
  ...
  int mxr = *std::max_element(rs, rs + n);
  int mxc = *std::max_element(cs, cs + m);
  if (mxc > mxr)
  {
    auto bci = std::distance(cs, std::max_element(cs, cs + m));
    for (int y = 0; y < n; y++)
    {
      int s{};
      for (int x = 0; x < m; x++)
        if (grid[y][x] == mx && x != bci)
          s += 1;
  ...
  {
    auto bri = std::distance(rs, std::max_element(rs, rs + n));

    for (int x = 0; x < m; x++)
    {
      int s{};
      for (int y = 0; y < n; y++)
        if (grid[y][x] == mx && y != bri)
          s += 1;
