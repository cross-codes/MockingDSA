...
void fill(int64_t offset[], int64_t from, int n)
{
  for (int i = 1; i <= n; i++)
    offset[i - 1] =
        i * static_cast<int>(std::ceil(static_cast<double>(from) / i));
}
...
auto run() -> void
{
  ...
  int64_t res[n], mult[n], offset[n];
  std::memset(mult, 0x0, sizeof(int64_t) * n);
  std::memset(offset, 0x0, sizeof(int64_t) * n);
  fill(offset, l, n);

  for (int i = 0; i < n; i++)
  {
    res[i] = offset[i] + mult[i] * (i + 1);
    mult[i] += 1;
