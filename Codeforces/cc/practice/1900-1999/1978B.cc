...
auto f(long double k, int64_t b, int64_t n, int64_t a) -> long double
{
  return k * (b + 1) - ((k * (k + 1)) / 2) + (n - k) * a;
}

auto unimodal_max(long double l, long double r, int64_t b, int64_t n, int64_t a)
    -> int64_t
{
  while (r - l >= 3)
  {
  ...
  int L{static_cast<int>(std::floor(l))};

  int64_t max_val = INT_MIN;

  for (int i = L; i <= R; i++)
    max_val = std::max(max_val, static_cast<int64_t>(f(i, b, n, a)));
...
auto run() -> void
{
  ...
  io::cout << unimodal_max(0, std::min(n, b), b, n, a) << "\n";
}
