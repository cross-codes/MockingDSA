...
auto run() -> void
{
  ...
  auto MIN_SELECT = [](const std::pair<int, int> &a,
                       const std::pair<int, int> &b) -> std::pair<int, int> {
    if (a.first < b.first)
      return a;
    else
      return b;
  ...
  auto MAX_SELECT = [](const std::pair<int, int> &a,
                       const std::pair<int, int> &b) -> std::pair<int, int> {
    if (a.first < b.first)
      return b;
    else
  ...
  IdempotentSparseTable<std::pair<int, int>> max_table(MAX_SELECT, a, n);
  IdempotentSparseTable<std::pair<int, int>> min_table(MIN_SELECT, a, n);
  ...
  while (q-- > 0)
  {
    ...
    auto mn = min_table.query_range(l - 1, r);
    auto mx = max_table.query_range(l - 1, r);

    if (mn.first == mx.first)
      io::cout << "-1 -1\n";
    else
      io::cout.append<"% %\n">(mn.second + 1, mx.second + 1);
  }
