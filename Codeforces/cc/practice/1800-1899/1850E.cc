...
auto run() -> void
{
  ...
  __int128_t c = static_cast<__int128_t>(c_64);
  __int128_t s[n];
  __int128_t sum_i{}, sum_i_2{};
  for (int i = 0; i < n; i++)
  {
    int64_t num;
    io::cin >> num;
  ...
  __int128_t δ = (16 * (sum_i * sum_i)) - 16 * n * (sum_i_2 - c);
  __int128_t res =
      static_cast<__int128_t>(std::sqrtl(static_cast<long double>(δ)));
  io::cout << static_cast<int64_t>((res - (4 * sum_i)) / (8 * n)) << "\n";
}
