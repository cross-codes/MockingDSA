...
auto f(int64_t k, int a[], int b[], int n, int m) -> int64_t
{
  int64_t total_cost{};
  for (int i = 0; i < n; i++)
    total_cost += std::max(0LL, k - a[i]);

  for (int i = 0; i < m; i++)
    total_cost += std::max(b[i] - k, 0LL);

  return total_cost;
}

auto unimodal_min(int a[], int b[], int n, int m, int64_t l, int64_t r)
    -> int64_t
{
  while (r - l > 3)
  {
  ...
  for (int64_t i = l; i <= r; i++)
    min_val = std::min(min_val, f(i, a, b, n, m));
...
auto run() -> void
{
  ...
  if (a[0] >= b[m - 1])
    ...
    io::cout << unimodal_min(a, b, n, m, 1, b[m - 1]) << "\n";
}
