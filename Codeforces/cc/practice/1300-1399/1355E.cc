...
auto f(int height, int h[], int n, int a, int r, int m) -> int64_t
{
  int64_t P{}, Q{};
  for (int i = 0; i < n; i++)
  {
    P += std::max(0, height - h[i]);
    Q += std::max(0, h[i] - height);
  }
...
auto unimodal_min(int h[], int n, int a, int r, int m, int L, int R) -> int64_t
{
  while (R - L >= 3)
  {
    int m1 = L + (R - L) / 3;
    ...
    if (f(m1, h, n, a, r, m) > f(m2, h, n, a, r, m))
      L = m1;
    else
      R = m2;
  }
  ...
  for (int i = L; i <= R; i++)
    min_val = std::min(min_val, f(i, h, n, a, r, m));
...
auto run() -> void
{
  ...
  int min_val = *std::min_element(h, h + n);
  int max_val = *std::max_element(h, h + n);

  io::cout << unimodal_min(h, n, a, r, m, min_val, max_val) << "\n";
}
