...
public class _1704B implements Runnable {

  InputReader in;
  OutputWriter out;

  public static void main(String[] args) {
    new Thread(null, new _1704B(), "", 256 * (1L << 20)).start();
  }
  ...
  void solve() {
    ...
    while (t-- > 0) {
      ...
      int v = arr[0], cnt = 0;
      if (arr[1] > arr[0]) v = arr[0] + x;
      else if (arr[1] < arr[0]) v = arr[0] - x;
      for (int i = 0; i < n - 1; i++) {
        if (Math.abs(v - arr[i]) > x) {
          if (arr[i + 1] > arr[i]) v = arr[i] + x;
          else if (arr[i + 1] < arr[i]) v = arr[i] - x;
          else v = arr[i];
        }
      }
