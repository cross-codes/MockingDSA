...
public:
  ...
  void increment_by(int amt)
  {
    x_ += amt;
  }

  void decrement_by(int amt)
  {
    x_ -= amt;
  }
...
auto run() -> void
{
  ...
  std::vector<std::shared_ptr<Marker>> array[N + 1];
  ...
  for (int i = 0; i < M; i++)
  {
    ...
    auto day = std::make_shared<Marker>();
    day->increment_by(k);
  ...
  for (int i = 0; i < N; i++)
  {
    ...
    for (const auto &marker : array[b])
    {
      marker->decrement_by(1);
      if (marker->nulled())
        overcome++;
    }
