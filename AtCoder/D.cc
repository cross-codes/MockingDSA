...
auto run() -> void
{
  ...
  int num_o_left = K - o_cnt;

  auto count_x   = std::count(res.begin(), res.end(), 'X');
  if (count_x <= num_o_left)
  {
    for (int i = 0; i < N; i++)
    {
      if (res[i] == 'X')
  ...
  {
    std::map<int, std::pair<int, Parity>> seg_greedy{};
    ...
    for (int i = 0; i < N; i++)
    {
      if (in_window)
      {
        if (res[i] != 'X')
        {
          seg_greedy[seg_start] = std::make_pair(
              (seg_end - seg_start + 2) >> 1,
              (seg_end - seg_start + 1) % 2 == 0 ? Parity::EVEN : Parity::ODD);
          seg_start = seg_end + 1;
          in_window = false;
        }
        else
    ...
    for (int i = 0; i < N; i++)
    {
      if (res[i] == 'X')
      {
        auto it = seg_greedy.lower_bound(i);
        if (it == seg_greedy.end())
          it = std::prev(seg_greedy.end());

        int start    = it->first, total{};
        bool at_even = (i - start) % 2 != 0;
        for (const auto &[s_start, max] : seg_greedy)
        {
          if (s_start == start)
            continue;
          else
        ...
        if (it->second.second == Parity::ODD)
        {
          if (total + it->second.first - 1 >= num_o_left)
            res[i] = '?';
          else
            res[i] = (at_even) ? '.' : 'o';
        }
        else if (total + it->second.first >= num_o_left)
          res[i] = '?';
      }
    }
  }
