...
auto run() -> void
{
  ...
  int x, y, g;
  Algebra::solveDiophantine(9, 4, Y - N, x, y, g);

  double llim1{(static_cast<double>(x) + y - N) / 5};
  double llim2{(static_cast<double>(-x)) / 4};

  double llim = std::max(llim1, llim2);
  double ulim{(static_cast<double>(y) / 9)};

  int k{};
  if (std::floor(ulim) - std::ceil(llim) >= 0)
    k = random.nextUniformInteger<int>(static_cast<int>(std::ceil(llim)),
                                       static_cast<int>(std::floor(ulim)));
  else
  {
    std::cout << "-1 -1 -1\n";
    return;
