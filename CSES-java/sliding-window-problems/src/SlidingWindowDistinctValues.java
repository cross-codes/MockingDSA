...
public class SlidingWindowDistinctValues extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SlidingWindowDistinctValues(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    HashMap<Integer, Integer> contents = new HashMap<>(n);
    for (int i = 0; i < k; i++) {
      contents.merge(x[i], 1, Integer::sum);
    }
    out.append(contents.size()).append(" ");

    for (int i = k; i < n; i++) {
      contents.compute(x[i - k], (key, value) -> value == 1 ? null : value - 1);
      contents.merge(x[i], 1, Integer::sum);
      out.append(contents.size()).append(" ");
    }
...
@MultipleInheritanceDisallowed(inheritor = SlidingWindowDistinctValues.class)
