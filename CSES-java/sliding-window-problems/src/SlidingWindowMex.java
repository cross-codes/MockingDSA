...
public class SlidingWindowMex extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SlidingWindowMex(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    IntConsumer addElement = (int e) -> {
      int prev = count.getOrDefault(e, 0);
    ...
    IntConsumer removeElement = (int e) -> {
      int prev = count.get(e);
      count.put(e, prev - 1);
      if (prev == 1) {
        available.add(e);
      }
    ...
    out.append(available.first()).append(" ");
...
@MultipleInheritanceDisallowed(inheritor = SlidingWindowMex.class)
