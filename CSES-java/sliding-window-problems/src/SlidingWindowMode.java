...
public class SlidingWindowMode extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SlidingWindowMode(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt(), k = in.nextInt();
    int[] x = in.readIntegerArray(n);

    TreeMap<Integer, TreeSet<Integer>> inverseMapping = new TreeMap<>();
    HashMap<Integer, Integer> freq = new HashMap<>(n);

    IntConsumer addElement = (e) -> {
      int prev = freq.getOrDefault(e, 0);
      freq.merge(e, 1, Integer::sum);

      if (inverseMapping.containsKey(prev)) {
        var set = inverseMapping.get(prev);
        set.remove(e);
        if (set.size() == 0) {
          inverseMapping.remove(prev);
        }
      }

      inverseMapping.computeIfAbsent(prev + 1, key -> new TreeSet<>()).add(e);
    };

    IntConsumer removeElement = (e) -> {
      int curr = freq.get(e);
      freq.compute(e, (key, value) -> value == 1 ? null : value - 1);

      var set = inverseMapping.get(curr);
      set.remove(e);
      if (set.size() == 0) {
        inverseMapping.remove(curr);
      }

      if (curr > 1) {
        inverseMapping.computeIfAbsent(curr - 1, key -> new TreeSet<>()).add(e);
      }
    };

    for (int i = 0; i < k; i++) {
      addElement.accept(x[i]);
    }

    out.append(inverseMapping.lastEntry().getValue().first()).append(" ");

    for (int i = k; i < n; i++) {
      removeElement.accept(x[i - k]);
      addElement.accept(x[i]);
...
@MultipleInheritanceDisallowed(inheritor = SlidingWindowMode.class)
