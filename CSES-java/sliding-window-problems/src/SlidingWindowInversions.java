...
public class SlidingWindowInversions extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SlidingWindowInversions(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt(), k = in.nextInt();
    int[] x = new int[n];
    var ref = new IntegerOrderedPair(-1, Integer.MAX_VALUE);

    AVLTreeSet<IntegerOrderedPair> ost = new AVLTreeSet<>();
    long inversions = 0;
    for (int i = 0; i < k; i++) {
      x[i] = ref.first = in.nextInt();
      ost.add(new IntegerOrderedPair(x[i], i));
      inversions += ost.countHigher(ref);
    }

    out.append(inversions).append(" ");
    ...
    for (int i = k; i < n; i++) {
      ...
      inversions -= ost.countLower(ref);
      ost.remove(new IntegerOrderedPair(x[i - k], i - k));
...
@MultipleInheritanceDisallowed(inheritor = SlidingWindowInversions.class)
