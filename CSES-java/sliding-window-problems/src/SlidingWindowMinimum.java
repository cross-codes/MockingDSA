...
public class SlidingWindowMinimum extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SlidingWindowMinimum(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt(), k = in.nextInt();
    long[] params = in.readLongArray(4);

    BarrettReducer reducer = new BarrettReducer(params[3]);

    int[] x = new int[n];
    x[0] = (int) params[0];
    for (int i = 1; i < n; i++) {
      x[i] = (int) reducer.reduce(params[1] * x[i - 1] + params[2]);
    }

    ArrayDeque<Integer> deque = new ArrayDeque<>();
    for (int i = 0; i < k; i++) {
      while (!deque.isEmpty() && deque.peekLast() > x[i]) {
        deque.removeLast();
      }
    ...
    long res = deque.peekFirst();
    for (int i = k; i < n; i++) {
      if (x[i - k] == deque.peekFirst()) {
        deque.poll();
      }
...
@MultipleInheritanceDisallowed(inheritor = SlidingWindowMinimum.class)
