...
public class ListRemovals extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new ListRemovals(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int i = 1; i <= n; i++) {
      ost.add(new IntegerOrderedPair(i, in.nextInt()));
    }

    for (int i = 1; i <= n; i++) {
      int p = in.nextInt();
      var e = ost.findByOrder(p - 1);
      out.append(e.second).append(" ");
      ost.remove(e);
    }
...
@MultipleInheritanceDisallowed(inheritor = ListRemovals.class)
