...
public class HotelQueries extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new HotelQueries(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    SegmentTree tree = new SegmentTree(Long::max, h, Long.MIN_VALUE);

    Predicate pred = (q, v) -> {
      long f = tree.rangeQuery(0, q);
      return f >= v;
    };
    ...
    for (int i = 0; i < m; i++) {
      ...
      if (R == n + 1) {
        ...
        out.append(R).append(" ");
        tree.setAtIndex(R - 1, tree.rangeQuery(0, R) - r);
      }
    }
...
@MultipleInheritanceDisallowed(inheritor = HotelQueries.class)
abstract class ModuleSignatures {
  @FunctionalInterface
  interface Predicate {
    boolean test(int index, long val);
  }
}
