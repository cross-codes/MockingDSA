...
public class SalaryQueries extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SalaryQueries(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int i = 1; i <= n; i++) {
      ...
      ost.add(new IntegerOrderedPair(salary, i));
      p[i] = salary;
    }
    ...
    for (int i = 0; i < q; i++) {
      ...
      if (type[0] == (byte) '!') {
        ...
        p[k] = x;
        ost.add(new IntegerOrderedPair(x, k));
      } else {
        int a = in.nextInt(), b = in.nextInt();

        ref.first = a;
        ref.second = Integer.MIN_VALUE;
        int rankA = ost.countLower(ref);
        ref.first = b;
        ref.second = Integer.MAX_VALUE;
        int rankB = ost.countLower(ref);

        out.append(rankB - rankA).appendNewLine();
      }
    }
  }
...
@MultipleInheritanceDisallowed(inheritor = SalaryQueries.class)
...
class AVLTreeSet<E extends Comparable<E>> {
  private Item<E> root;
