...
public class RangeIntervalQueries extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new RangeIntervalQueries(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    IntegerIndexCompress compressor = new IntegerIndexCompress(x.clone());

    int[] xComp = new int[n];
    for (int i = 0; i < n; i++) {
      xComp[i] = compressor.getCompressedIndex(x[i]);
    }

    WaveletMatrix wmm = new WaveletMatrix(xComp, compressor.getMaxRange());

    for (int i = 0; i < q; i++) {
      int a = in.nextInt(), b = in.nextInt(), c = in.nextInt(), d = in.nextInt();
      int cmpMin = compressor.getCompressedIndex(c);
      int cmpMax = compressor.getCompressedIndex(d + 1) - 1;
      out.append(wmm.countInRange(a - 1, b, cmpMin, cmpMax)).appendNewLine();
    }
  }

}

@MultipleInheritanceDisallowed(inheritor = RangeIntervalQueries.class)
