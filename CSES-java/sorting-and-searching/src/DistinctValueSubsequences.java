...
public class DistinctValueSubsequences extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new DistinctValueSubsequences(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int i = 0; i < n; i++) {
      freq.merge(in.nextInt(), 1, Integer::sum);
    }
    ...
    out.append((res - 1 + MOD) % MOD).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = DistinctValueSubsequences.class)
