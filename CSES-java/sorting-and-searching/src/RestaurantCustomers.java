...
public class RestaurantCustomers extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new RestaurantCustomers(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    int n = in.nextInt();
    ArrayList<OrderedPair<Integer, TimeType>> times = new ArrayList<>(n);
    for (int i = 0; i < n; i++) {
      times.add(new OrderedPair<>(in.nextInt(), TimeType.INCOMING));
      times.add(new OrderedPair<>(in.nextInt(), TimeType.OUTGOING));
    }

    Collections.sort(times);
    int mx = Integer.MIN_VALUE, cur = 0;
    for (var pair : times) {
      if (pair.second == TimeType.OUTGOING) {
        cur -= 1;
      } else {
        cur += 1;
      }
...
@MultipleInheritanceDisallowed(inheritor = RestaurantCustomers.class)
