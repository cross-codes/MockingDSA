...
public class NearestSmallestValues extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new NearestSmallestValues(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();
    ArrayDeque<IntegerOrderedPair> stack = new ArrayDeque<>();

    for (int i = 1; i <= n; i++) {
      int x = in.nextInt();
      while (!stack.isEmpty() && stack.peekLast().first >= x) {
        stack.removeLast();
      }

      out.append(stack.isEmpty() ? 0 : stack.peekLast().second).append(" ");
      stack.addLast(new IntegerOrderedPair(x, i));
    }
...
@MultipleInheritanceDisallowed(inheritor = NearestSmallestValues.class)
