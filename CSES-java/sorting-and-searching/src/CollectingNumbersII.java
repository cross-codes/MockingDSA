...
public class CollectingNumbersII extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new CollectingNumbersII(), "LaunchableDriver", 1048576L).start();
  }

  @Override
  public Object[] initialPass(int[] numbers) {
    HashMap<Integer, Integer> pos = new HashMap<>(200000);
    int mx = 0;
    for (int i = 1; i <= numbers.length; i++) {
      pos.put(numbers[i - 1], i);
      mx = Math.max(mx, numbers[i - 1]);
    }

    int cur = 1, cnt = 1, idx = pos.get(cur);
    while (cur < mx) {
      while (cur < mx && pos.get(cur + 1) > idx) {
        idx = pos.get(cur + 1);
        cur += 1;
      }

      if (cur != mx && pos.get(cur + 1) < idx) {
        idx = pos.get(cur + 1);
        cur += 1;
        cnt += 1;
      }
    ...
    return new Object[] { pos, cnt };
  }

  @Override
  public int modifyConditional(int cnt, int cond) throws RuntimeException {
    switch (cond) {
      case 0b000:
        return cnt
            + ((posAMinusOld != null && posAMinusNew != null && posAMinusOld < oldAPos && posAMinusNew > newAPos) ? 1
                : 0);
      case 0b001:
        return cnt
            - ((posAMinusOld != null && posAMinusNew != null && posAMinusOld > oldAPos && posAMinusNew < newAPos) ? 1
      ...
      case 0b010:
        return cnt
            - ((posAPlusOld != null && posAPlusNew != null && posAPlusOld < oldAPos && posAPlusNew > newAPos) ? 1
                : 0);
      case 0b011:
        return cnt
            + ((posAPlusOld != null && posAPlusNew != null && posAPlusOld > oldAPos && posAPlusNew < newAPos) ? 1 : 0);
      case 0b100:
        return cnt
            + ((posBMinusOld != null && posBMinusNew != null && posBMinusOld < oldBPos && posBMinusNew > newBPos) ? 1
                : 0);
      case 0b101:
        return cnt
            - ((posBMinusOld != null && posBMinusNew != null && posBMinusOld > oldBPos && posBMinusNew < newBPos) ? 1
      ...
      case 0b110:
        return cnt
            - ((posBPlusOld != null && posBPlusNew != null && posBPlusOld < oldBPos && posBPlusNew > newBPos) ? 1 : 0);
      case 0b111:
        return cnt
            + ((posBPlusOld != null && posBPlusNew != null && posBPlusOld > oldBPos && posBPlusNew < newBPos) ? 1 : 0);
  ...
  private void solveCase(int _case) {
    ...
    Object[] values = initialPass(numbers);
    @SuppressWarnings("unchecked")
    HashMap<Integer, Integer> pos = (HashMap<Integer, Integer>) values[0];
    int cnt = (Integer) values[1];
    ...
    for (int i = 0; i < m; i++) {
      ...
      newAPos = oldBPos;
      newBPos = oldAPos;
      numA = numbers[oldAPos - 1];
      numB = numbers[oldBPos - 1];

      posAPlusOld = pos.get(numA + 1);
      posBPlusOld = pos.get(numB + 1);
      posAMinusOld = pos.get(numA - 1);
      posBMinusOld = pos.get(numB - 1);

      pos.put(numA, newAPos);
      pos.put(numB, newBPos);
      numbers[oldAPos - 1] = numB;
      numbers[oldBPos - 1] = numA;

      posAPlusNew = pos.get(numA + 1);
      posBPlusNew = pos.get(numB + 1);
      posAMinusNew = pos.get(numA - 1);
      posBMinusNew = pos.get(numB - 1);
...
@MultipleInheritanceDisallowed(inheritor = CollectingNumbersII.class)
abstract class ModuleSignatures {
  protected int oldAPos, oldBPos, newAPos, newBPos, numA, numB;
  protected Integer posAPlusOld, posBPlusOld, posAMinusOld, posBMinusOld;
  protected Integer posAPlusNew, posBPlusNew, posAMinusNew, posBMinusNew;

  abstract public int modifyConditional(int cnt, int type) throws RuntimeException;

  abstract public Object[] initialPass(int[] numbers);
}
