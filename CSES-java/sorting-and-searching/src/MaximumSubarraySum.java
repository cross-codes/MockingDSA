...
public class MaximumSubarraySum extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new MaximumSubarraySum(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();

    long cur = 0, mx = Long.MIN_VALUE, mxe = Long.MIN_VALUE;
    boolean all_neg = true;
    for (int i = 0; i < n; i++) {
      long x = in.nextLong();
      all_neg = all_neg && x < 0;
      mxe = Math.max(mxe, x);
      cur = Math.max(0, x + cur);
      mx = Math.max(cur, mx);
    }

    out.append(all_neg ? mxe : mx).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = MaximumSubarraySum.class)
