...
public class Playlist extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new Playlist(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    int mx = 0, cur = 0, p = 0;
    for (int r = 0; r < n; r++) {
      if (!seen.containsKey(ids[r])) {
        cur += 1;
      } else {
        int upto = seen.get(ids[r]) + 1;
...
@MultipleInheritanceDisallowed(inheritor = Playlist.class)
