...
public class SubarrayDivisibility extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SubarrayDivisibility(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();
    int[] remainderStore = new int[n];
    remainderStore[0] = 1;
    ...
    for (int i = 1; i <= n; i++) {
      ...
      int rem = Math.floorMod(prefix[i], n);
      res += remainderStore[rem];
      remainderStore[rem] += 1;
    }
...
@MultipleInheritanceDisallowed(inheritor = SubarrayDivisibility.class)
