...
public class TasksAndDeadlines extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new TasksAndDeadlines(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();
    ArrayList<IntegerOrderedPair> tasks = new ArrayList<>(n);
    for (int i = 0; i < n; i++) {
      tasks.add(new IntegerOrderedPair(in.nextInt(), in.nextInt()));
    }
    ...
    out.append(reward).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = TasksAndDeadlines.class)
