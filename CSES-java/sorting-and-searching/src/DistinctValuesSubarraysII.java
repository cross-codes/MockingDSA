...
public class DistinctValuesSubarraysII extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new DistinctValuesSubarraysII(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    int l = 0;
    for (int r = 0; r < n; r++) {
      if (pos.size() == k && !pos.containsKey(x[r])) {
        while (pos.size() != k - 1) {
          pos.computeIfPresent(x[l], (K, v) -> v == 1 ? null : v - 1);
          l += 1;
        }
      }

      cnt += r - l + 1;
      pos.merge(x[r], 1, Integer::sum);
    }
...
@MultipleInheritanceDisallowed(inheritor = DistinctValuesSubarraysII.class)
