...
public class RoomAllocation extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new RoomAllocation(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    ArrayList<OrderedTriplet<Integer, TimeType, Integer>> intervals = new ArrayList<>(n);
    for (int i = 0; i < n; i++) {
      int a = in.nextInt(), b = in.nextInt();
      intervals.add(new OrderedTriplet<>(a, TimeType.INCOMING, i));
      intervals.add(new OrderedTriplet<>(b, TimeType.OUTGOING, i));
    }

    intervals.sort(Comparator.naturalOrder());

    ArrayDeque<Integer> freeRooms = new ArrayDeque<>();
    int[] allotmentHistory = new int[n];
    int cur = 0, mx = 0, nxtRoom = 1;
    for (var e : intervals) {
      if (e.second == TimeType.INCOMING) {
        cur += 1;
        int room = freeRooms.isEmpty() ? nxtRoom++ : freeRooms.pop();
        allotmentHistory[e.third] = room;
      } else {
        cur -= 1;
        int room = allotmentHistory[e.third];
        freeRooms.push(room);
      }
...
@MultipleInheritanceDisallowed(inheritor = RoomAllocation.class)
...
class OrderedTriplet<F extends Comparable<F>, S extends Comparable<S>, T extends Comparable<T>>
