...
public class SumOfTwoValues extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SumOfTwoValues(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int i = 1; i <= n; i++) {
      int a = in.nextInt();
      if (seen.containsKey(x - a)) {
        out.append(i).append(" ").append(seen.get(x - a)).appendNewLine();
        return;
      }
...
@MultipleInheritanceDisallowed(inheritor = SumOfTwoValues.class)
