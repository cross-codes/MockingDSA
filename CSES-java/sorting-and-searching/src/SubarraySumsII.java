...
public class SubarraySumsII extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SubarraySumsII(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt(), x = in.nextInt();
    HashMap<Long, Integer> prefixStore = new HashMap<>();
    prefixStore.put(0L, 1);
    ...
    for (int i = 1; i <= n; i++) {
      ...
      res += prefixStore.getOrDefault(prefix[i] - x, 0);
      prefixStore.merge(prefix[i], 1, Integer::sum);
    }
...
@MultipleInheritanceDisallowed(inheritor = SubarraySumsII.class)
