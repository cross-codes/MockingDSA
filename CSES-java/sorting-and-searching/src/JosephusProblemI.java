...
public class JosephusProblemI extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new JosephusProblemI(), "LaunchableDriver", 1048576L).start();
  }

  private static class DoublyLinkedNode {
    public int value;
    public DoublyLinkedNode prev = null, next = null;
  ...
  private void solveCase(int _case) {
    int n = in.nextInt();
    DoublyLinkedNode head = new DoublyLinkedNode(1), curr = head;
    for (int i = 2; i <= n; i++) {
      DoublyLinkedNode nextNode = new DoublyLinkedNode(i);
      curr.next = nextNode;
      nextNode.prev = curr;
      curr = nextNode;
    }
    ...
    while (curr.next != curr || curr.prev != curr) {
      DoublyLinkedNode rep = curr.next;
      out.append(rep.value).append(" ");
      curr.next = rep.next;
      rep.next.prev = curr;
      curr = curr.next;
    }

    out.append(curr.value).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = JosephusProblemI.class)
