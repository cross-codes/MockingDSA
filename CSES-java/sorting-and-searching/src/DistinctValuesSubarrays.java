...
public class DistinctValuesSubarrays extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new DistinctValuesSubarrays(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    int l = 0;
    for (int r = 0; r < n; r++) {
      if (pos.containsKey(x[r])) {
        int upto = pos.get(x[r]) + 1;
        for (; l < upto; l++) {
          pos.remove(x[l]);
        }
      }
...
@MultipleInheritanceDisallowed(inheritor = DistinctValuesSubarrays.class)
