...
public class SumOfThreeValues extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SumOfThreeValues(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int i = 0; i < n; i++) {
      int l = i + 1, r = n - 1, target = x - a.get(i).first;
      while (l < r) {
        if (a.get(l).first + a.get(r).first < target) {
          l += 1;
        } else if (a.get(l).first + a.get(r).first > target) {
          r -= 1;
        } else {
          out.append(String.format("%d %d %d", a.get(i).second, a.get(l).second, a.get(r).second))
              .appendNewLine();
          return;
        }
      }
...
@MultipleInheritanceDisallowed(inheritor = SumOfThreeValues.class)
