...
public class ArrayDivision extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new ArrayDivision(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    long L = Array.max(x) - 1, R = (((long) (1e14)) << 1 + 1);
    while (R - L > 1) {
      long M = L + ((R - L) >>> 1);
      if (pred.test(M)) {
        R = M;
    ...
    out.append(R).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = ArrayDivision.class)
