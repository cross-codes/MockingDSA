...
public class MinimizingCoins extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new MinimizingCoins(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    int[] minc = new int[1000001];
    minc[0] = 0;
    for (int i = 1; i <= x; i++) {
      minc[i] = Integer.MAX_VALUE;
      for (int c: coins) {
        if (i >= c && minc[i - c] != Integer.MAX_VALUE) {
          minc[i] = Math.min(minc[i - c] + 1, minc[i]);
        }
      }
    }

    out.append(minc[x] == Integer.MAX_VALUE ? -1 : minc[x]).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = MinimizingCoins.class)
