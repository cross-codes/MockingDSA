...
public class GridPathsI extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new GridPathsI(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    final int MOD = 1000000007;
    int n = in.nextInt();
    byte[][] grid = new byte[n + 1][n + 1];
    for (int y = 1; y <= n; y++) {
      for (int x = 1; x <= n; x++) {
    ...
    for (int y = 1; y <= n; y++) {
      for (int x = 1; x <= n; x++) {
        if (grid[y][x] != '*') {
          ...
          ways[y][x] = Math.floorMod(ways[y][x], MOD);
        }
      }
    }

    out.append(ways[n][n]).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = GridPathsI.class)
