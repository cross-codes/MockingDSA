...
public class BookShop extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new BookShop(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int i = 1; i <= n; i++) {
      for (int j = x; j >= h[i]; j--) {
        mx[j] = Math.max(mx[j], mx[j - h[i]] + s[i]);
      }
    }
...
@MultipleInheritanceDisallowed(inheritor = BookShop.class)
