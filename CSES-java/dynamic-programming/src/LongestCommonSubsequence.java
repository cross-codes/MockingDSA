...
public class LongestCommonSubsequence extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new LongestCommonSubsequence(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    out.append(c[n][m]).appendNewLine();
    int[] seq = new int[c[n][m]];
    int y = n, x = m, i = c[n][m] - 1;
    while (d[y][x] != d[0][0]) {
      if (d[y][x] == 'U') {
        y -= 1;
      } else if (d[y][x] == 'L') {
...
@MultipleInheritanceDisallowed(inheritor = LongestCommonSubsequence.class)
