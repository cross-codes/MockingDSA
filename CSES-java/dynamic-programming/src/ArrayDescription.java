...
public class ArrayDescription extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new ArrayDescription(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    long[][] count = new long[100001][101];
    if (x[1] == 0) {
      for (int i = 1; i <= m; i++) {
        count[1][i] = 1;
      }
    ...
    out.append(total % MOD).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = ArrayDescription.class)
