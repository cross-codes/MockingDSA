...
public class EditDistance extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new EditDistance(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    byte[] s = in.nextLine(), t = in.nextLine();
    ...
    for (int y = 1; y <= n; y++) {
      for (int x = 1; x <= m; x++) {
        int remove = e[y - 1][x] + 1;
        int add = e[y][x - 1] + 1;
        int modify = e[y - 1][x - 1] + ((s[y - 1] == t[x - 1]) ? 0 : 1);
        e[y][x] = Math.min(remove, Math.min(add, modify));
      }
    }

    out.append(e[n][m]).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = EditDistance.class)
