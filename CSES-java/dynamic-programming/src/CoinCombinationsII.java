...
public class CoinCombinationsII extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new CoinCombinationsII(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    final int MOD = 1000000007;
    int n = in.nextInt(), x = in.nextInt();
    int[] c = in.readIntegerArray(n);
    Arrays.sort(c);

    int[] ways = new int[1000001];
    ways[0] = 1;
    for (int e : c) {
      for (int i = e; i < 1000001; i++) {
        ways[i] += ways[i - e];
        ways[i] %= MOD;
      }
    }

    out.append(ways[x]).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = CoinCombinationsII.class)
