...
public class RemovingDigits extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new RemovingDigits(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();

    IntegerFunction highestDigit = (u) -> {
      int mx = 0;
      while (u != 0) {
        mx = Math.max(mx, u % 10);
        u /= 10;
    ...
    int steps = 0;
    while (n != 0) {
      n -= highestDigit.apply(n);
      steps += 1;
    }

    out.append(steps).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = RemovingDigits.class)
