...
public class TwoKnights extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new TwoKnights(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int k = 1; k <= n; k++) {
      long k2 = k * k;
      long threaten = ((k - 1) * (k - 2)) << 2;
      long total = (k2 * (k2 - 1)) >> 1;
      out.append(total - threaten).appendNewLine();
    }
  }

}

@MultipleInheritanceDisallowed(inheritor = TwoKnights.class)
