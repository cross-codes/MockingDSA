...
public class DigitQueries extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new DigitQueries(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    long k = in.nextLong();
    int n = 1;
    long sum = 0, prevSum = 0, currRange = 9;
    ...
    n -= 1;
    long start = (long) Math.pow(10L, n - 1);
    long skip = (k - prevSum - 1) / n;
    int chSkip = Math.floorMod(k - prevSum - 1, n);
    out.append(Long.toString(start + skip).charAt(chSkip)).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = DigitQueries.class)
