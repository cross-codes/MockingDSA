...
public class KnightMovesGrid extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new KnightMovesGrid(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int y = 0; y < n; y++) {
      Arrays.fill(moves[y], Integer.MAX_VALUE);
    }
    ...
    queue.add(new IntegerOrderedTriplet(0, 0, 0));
    while (!queue.isEmpty()) {
      var curr = queue.poll();
      int y = curr.first, x = curr.second, cnt = curr.third;

      moves[y][x] = Math.min(moves[y][x], cnt);
      for (int i = 0; i < 8; i++) {
        int Y = y + dy[i], X = x + dx[i];
        if (inBounds.test(Y, X) && moves[Y][X] == Integer.MAX_VALUE) {
          moves[Y][X] = cnt + 1;
          queue.add(new IntegerOrderedTriplet(Y, X, cnt + 1));
        }
      }
    }
...
@MultipleInheritanceDisallowed(inheritor = KnightMovesGrid.class)
