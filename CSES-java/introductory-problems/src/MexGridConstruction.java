...
public class MexGridConstruction extends ModuleSignatures implements Runnable {
  ...
  private void solveCase(int _case) {
    ...
    HashSet<Integer> range = IntStream.range(0, 251).boxed().collect(Collectors.toCollection(HashSet::new));

    for (int[] y = { 0 }; y[0] < n; y[0]++) {
      for (int[] x = { 0 }; x[0] < n; x[0]++) {
        int target = range.stream()
            .filter(element -> !(rowSets.get(y[0]).contains(element) || colSets.get(x[0]).contains(element)))
            .min(Comparator.naturalOrder()).get();

        rowSets.get(y[0]).add(target);
        colSets.get(x[0]).add(target);
