...
public class GrayCode extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new GrayCode(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int i = 0; i < (1 << n); i++) {
      int gray = i ^ (i >>> 1);
      long rep = Long.parseLong(Integer.toBinaryString(gray), 10);
      out.append(String.format(fmt, rep)).appendNewLine();
    }
  }

}

@MultipleInheritanceDisallowed(inheritor = GrayCode.class)
