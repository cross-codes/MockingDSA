...
public class AppleDivision extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new AppleDivision(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    long subsetSum, bestDiff = Long.MAX_VALUE;
    for (int mask = 0; mask < (1 << n); mask++) {
      ...
      bestDiff = Math.min(bestDiff, Math.abs(tot - (subsetSum << 1)));
    }

    out.append(bestDiff).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = AppleDivision.class)
