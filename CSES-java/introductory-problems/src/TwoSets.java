...
public class TwoSets extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new TwoSets(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt();
    int mod = Math.floorMod(n, 4);

    if (mod == 0) {
      out.append("YES").appendNewLine();
      var nums = IntStream.rangeClosed(1, n).toArray();
      ArrayList<Integer> set1 = new ArrayList<>(), set2 = new ArrayList<>();

      boolean addToFirstSet = false;
      for (int i = 0; i < (nums.length >> 1); i++) {
        if (addToFirstSet) {
          set1.add(nums[i]);
          set1.add(nums[nums.length - i - 1]);
        } else {
          set2.add(nums[i]);
          set2.add(nums[nums.length - i - 1]);
        }

        addToFirstSet = !addToFirstSet;
      }

      out.append(set1.size()).appendNewLine();
      for (int e : set1) {
        out.append(e).append(" ");
      }
      ...
      out.append(set2.size()).appendNewLine();
      for (int e : set2) {
        out.append(e).append(" ");
      }
      ...
      out.append("YES").appendNewLine();
      int m = (1 + n) >> 2;
      var nums = IntStream.rangeClosed(1, n).filter((a) -> (a != m && a != (m << 1) && a != (m << 1) + m)).toArray();
      ArrayList<Integer> set1 = new ArrayList<>(), set2 = new ArrayList<>();
      set1.add(m);
      set1.add(m << 1);
      set2.add((m << 1) + m);
...
@MultipleInheritanceDisallowed(inheritor = TwoSets.class)
