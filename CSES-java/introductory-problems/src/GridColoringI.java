...
public class GridColoringI extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new GridColoringI(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    byte[] choices = { 'A', 'B', 'C', 'D' };
    for (int y = 0; y < n; y++) {
      for (int x = 0; x < m; x++) {
        byte curr = grid[y][x];
        byte left = (x > 0 ? grid[y][x - 1] : (byte) '#'), up = (y > 0 ? grid[y - 1][x] : (byte) '#');
        boolean set = false;
        for (byte b : choices) {
          if (b != curr && b != left && b != up) {
            grid[y][x] = b;
            set = true;
            break;
          }
...
@MultipleInheritanceDisallowed(inheritor = GridColoringI.class)
