...
public class CountingGrids extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new CountingGrids(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    final int MOD = 1000000007;
    final int FOUR_INV = Algebra.modInverse(4, MOD);
    int n = in.nextInt();

    if ((n & 1) == 0) {
      int cycle_1_exp = Algebra.modPow(n, 2, MOD - 1);
      int cycle_2_exp = Algebra.modPow(n >> 1, 2, MOD - 1);
      int cycle_3_exp = Math.floorMod((cycle_2_exp << 1), MOD - 1);

      long res = Algebra.modPow(2, cycle_1_exp, MOD);
      res += ((long) Algebra.modPow(2, cycle_2_exp, MOD)) << 1;
      res += Algebra.modPow(2, cycle_3_exp, MOD);
      res *= FOUR_INV;

      out.append(Math.floorMod(res, MOD)).appendNewLine();
    } else {
      int k = (n - 1) >> 1;

      int cycle_1_exp = Algebra.modPow(n, 2, MOD - 1);
      int cycle_2_exp = Math.floorMod((long) k * (k + 1), MOD - 1);
      int cycle_3_exp = Math.floorMod((((long) k * (k + 1)) << 1), MOD - 1);

      long res = Algebra.modPow(2, cycle_1_exp, MOD);
      res += ((long) Algebra.modPow(2, cycle_2_exp, MOD)) << 2;
      res += Algebra.modPow(2, cycle_3_exp, MOD) << 1;
      res *= FOUR_INV;
...
@MultipleInheritanceDisallowed(inheritor = CountingGrids.class)
