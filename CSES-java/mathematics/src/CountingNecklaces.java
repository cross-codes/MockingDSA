...
public class CountingNecklaces extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new CountingNecklaces(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    long sum = Algebra.modPow(m, n, MOD);
    for (int i = 1; i < n; i++) {
      sum += Algebra.modPow(m, Algebra.gcd(i, n), MOD);
    }

    sum %= MOD;
    sum *= Algebra.modInverse(n, MOD);
    out.append(sum % MOD).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = CountingNecklaces.class)
