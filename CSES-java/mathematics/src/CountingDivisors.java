...
public class CountingDivisors extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new CountingDivisors(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int x = in.nextInt();

    IntFunction numFactors = (n) -> {
      int cnt = 0;
      for (int i = 1; i * i <= n; i++) {
        if (n % i == 0) {
          cnt += (i == n / i) ? 1 : 2;
    ...
    out.append(numFactors.apply(x)).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = CountingDivisors.class)
