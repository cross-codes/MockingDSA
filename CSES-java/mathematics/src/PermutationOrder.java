...
public class PermutationOrder extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new PermutationOrder(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {

    LongToSequenceFunction kthPermutation = (n, k) -> {
      int[] usedMask = { 0x0001 };

      IntegerFunction unused = (ord) -> {
        for (int i = 1; i <= n; i++) {
          if (((usedMask[0] >> i) & 1) == 0) {
            ord -= 1;
            if (ord <= 0) {
      ...
      for (int i = 0; i < n; i++) {
        int skip = (int) ((k + super.factorial[n - i - 1] - 1) / super.factorial[n - i - 1]);
        result[i] = unused.apply(skip);
        usedMask[0] |= (1 << result[i]);
        k -= (super.factorial[n - i - 1] * (skip - 1));
      }

      return result;
    };

    SequenceToLongFunction permutationIndex = (n, seq) -> {
      int[] usedMask = { 0x0001 };

      IntSupplier unused = () -> {
        return Integer.numberOfTrailingZeros(~usedMask[0]);
      };

      IntegerFunction ord = (pos) -> {
        int lmsk = (1 << pos) - 1;
        return pos - Integer.bitCount(usedMask[0] & lmsk);
      };

      long idx = 0;
      for (int i = 0; i < n; i++) {
        int target = unused.getAsInt();
        idx += (ord.apply(seq[i]) - ord.apply(target)) * super.factorial[n - i - 1];
        usedMask[0] |= (1 << seq[i]);
      }
    ...
    if (type == 1) {
      ...
      int[] seq = kthPermutation.get(n, k);
      for (int e : seq) {
        out.append(e).append(" ");
      }
      ...
      int n = in.nextInt();
      int[] p = in.readIntegerArray(n);

      out.append(permutationIndex.calculate(n, p)).appendNewLine();
    }
  }

}

@MultipleInheritanceDisallowed(inheritor = PermutationOrder.class)
abstract class ModuleSignatures {
  public long[] factorial = new long[21];
  ...
  @FunctionalInterface
  interface LongToSequenceFunction {
    int[] get(int n, long k);
  }
  ...
  @FunctionalInterface
  interface SequenceToLongFunction {
    long calculate(int n, int[] seq);
  }
}
