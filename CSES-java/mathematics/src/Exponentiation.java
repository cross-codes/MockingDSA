...
public class Exponentiation extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new Exponentiation(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    IntBinaryOperator modPow = (n, exponent) -> {
      long result = 1;
      for (long i = 1, j = n; i <= exponent; i <<= 1, j = j * j % MOD) {
        if ((i & exponent) != 0) {
          result = result * j % MOD;
        }
      }
    ...
    out.append(modPow.applyAsInt(a, b)).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = Exponentiation.class)
