...
public class DistributingApples extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new DistributingApples(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    int[] factorial = new int[1000001 << 1], inverseFactorial = new int[1000001 << 1];
    ...
    int res = Math.floorMod(
        Math.floorMod(factorial[n + m - 1] * (long) inverseFactorial[m], MOD) * (long) inverseFactorial[n - 1], MOD);
    out.append(res).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = DistributingApples.class)
