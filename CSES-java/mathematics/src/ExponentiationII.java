...
public class ExponentiationII extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new ExponentiationII(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    IntTernaryOperator modPow = (n, exponent, modulus) -> {
      long result = 1;
      for (long i = 1, j = n; i <= exponent; i <<= 1, j = j * j % modulus) {
        if ((i & exponent) != 0) {
          result = result * j % modulus;
        }
      }
    ...
    out.append(modPow.applyAsInt(a, modPow.applyAsInt(b, c, MOD - 1), MOD)).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = ExponentiationII.class)
