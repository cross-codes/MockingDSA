...
public class PoliceChase extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new PoliceChase(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int i = 0; i < m; i++) {
      ...
      Edge forwardRev = new Edge(b, a, 1, EdgeType.FORWARD);
      Edge backwardRev = new Edge(a, b, 0, EdgeType.BACKWARD);
      adj[a].add(backwardRev);
      adj[b].add(forwardRev);
      edgeList.add(forwardRev);
    }

    Edge[] Eparent = new Edge[n];
    int[] visited = new int[n], iteration = { 0 };
    ResettableIntQueue queue = new ResettableIntQueue(n);

    LongSupplier bottleneck = () -> {
      long mn = Long.MAX_VALUE;
      for (int v = n - 1; v != 0; v = Eparent[v].from) {
    ...
    DFS dfs = (instance, u) -> {
      belongsToA[u] = true;
      for (Edge e : adj[u]) {
        if (e.residualCapacity() > 0 && !belongsToA[e.to]) {
          instance.run(instance, e.to);
        }
      }
    };
    ...
    for (Edge e : edgeList) {
      int u = e.from, v = e.to;
      if (belongsToA[u] && !belongsToA[v]) {
        out.append(u + 1).append(" ").append(v + 1).appendNewLine();
      }
    }
  }
...
@MultipleInheritanceDisallowed(inheritor = PoliceChase.class)
