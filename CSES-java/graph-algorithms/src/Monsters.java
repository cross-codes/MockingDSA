...
public class Monsters extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new Monsters(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    Queue<IntegerOrderedTriplet> queue = new ArrayDeque<>();
    byte[][] grid = new byte[n][m];
    IntegerOrderedTriplet playerParams = new IntegerOrderedTriplet(-1, -1, -1);
    for (int y = 0; y < n; y++) {
      for (int x = 0; x < m; x++) {
        grid[y][x] = in.nextByte();
        if (grid[y][x] == (byte) 'M') {
          queue.add(new IntegerOrderedTriplet(y, x, Character.MONSTER.ordinal()));
          visited[y][x] = true;
        } else if (grid[y][x] == (byte) 'A') {
          playerParams.first = y;
          playerParams.second = x;
          playerParams.third = Character.PLAYER.ordinal();
        }
      }
    }

    IntegerBiPredicate inBounds = (y, x) -> {
      return y < n && y >= 0 && x < m && x >= 0;
    };

    IntegerBiPredicate isBoundary = (y, x) -> {
    ...
    queue.add(new IntegerOrderedTriplet(playerParams.first, playerParams.second, playerParams.third));
    IntegerOrderedPair[][] parent = new IntegerOrderedPair[n][m];
    IntegerOrderedPair exit = new IntegerOrderedPair(-1, -1);
    boolean found = false;
    while (!queue.isEmpty()) {
      var params = queue.poll();
      int y = params.first, x = params.second, type = params.third;

      if (type == Character.MONSTER.ordinal()) {
        for (int i = 0; i < 4; i++) {
          int X = x + dx[i], Y = y + dy[i];
          if (inBounds.test(Y, X) && grid[Y][X] == (byte) '.' && !visited[Y][X]) {
            visited[Y][X] = true;
            queue.add(new IntegerOrderedTriplet(Y, X, Character.MONSTER.ordinal()));
          }
        }
      } else {
        if (isBoundary.test(y, x)) {
          exit.first = y;
          exit.second = x;
          found = true;
          break;
          ...
          for (int i = 0; i < 4; i++) {
            ...
            if (inBounds.test(Y, X) && grid[Y][X] == (byte) '.' && !visited[Y][X]) {
              visited[Y][X] = true;
              parent[Y][X] = new IntegerOrderedPair(y, x);
              queue.add(new IntegerOrderedTriplet(Y, X, Character.PLAYER.ordinal()));
            }
          }
        }
      }
    ...
    IntegerArrayList trace = new IntegerArrayList(n * n);
    var curr = exit;

    PairPairToCharFunction directionInv = (IntegerOrderedPair from, IntegerOrderedPair to) -> {
      int y1 = from.first, x1 = from.second;
      int y2 = to.first, x2 = to.second;
    ...
    while (parent[curr.first][curr.second] != null) {
      var next = parent[curr.first][curr.second];
      trace.add(directionInv.get(curr, next));
      curr = next;
    }

    int sz = trace.size();
    out.append("YES").appendNewLine();
    out.append(sz).appendNewLine();

    for (int i = sz - 1; i >= 0; i--) {
      out.append((char) trace.get(i));
    }
...
@MultipleInheritanceDisallowed(inheritor = Monsters.class)
abstract class ModuleSignatures {
  ...
  @FunctionalInterface
  interface IntegerBiPredicate {
    boolean test(int y, int x);
  }

  @FunctionalInterface
  interface PairPairToCharFunction {
    char get(IntegerOrderedPair from, IntegerOrderedPair to);
  }
}
