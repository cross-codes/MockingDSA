...
public class SchoolDance extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new SchoolDance(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    ArrayList<Edge>[] adj = new ArrayList[n + m + 2];
    ...
    for (int i = 0; i < k; i++) {
      int a = in.nextInt(), b = in.nextInt() + n;
      Edge forward = new Edge(a, b, 1, EdgeType.FORWARD);
      Edge backward = new Edge(b, a, 0, EdgeType.BACKWARD);
    ...
    for (int a = 1; a <= n; a++) {
      Edge srcForward = new Edge(0, a, 1, EdgeType.FORWARD);
      Edge srcBackward = new Edge(a, 0, 0, EdgeType.BACKWARD);
      srcForward.rev = srcBackward;
      srcBackward.rev = srcForward;
      adj[0].add(srcForward);
      adj[a].add(srcBackward);
    }

    for (int b = n + 1; b <= n + m; b++) {
      Edge sinkForward = new Edge(b, n + m + 1, 1, EdgeType.FORWARD);
      Edge sinkBackward = new Edge(n + m + 1, b, 0, EdgeType.BACKWARD);
      sinkForward.rev = sinkBackward;
      sinkBackward.rev = sinkForward;
      adj[b].add(sinkForward);
      adj[n + m + 1].add(sinkBackward);
    }

    Edge[] Eparent = new Edge[n + m + 2];
    int[] visited = new int[n + m + 2], iteration = { 0 };
    ResettableIntQueue queue = new ResettableIntQueue(n + m + 2);

    LongSupplier bottleneck = () -> {
      long mn = Long.MAX_VALUE;
      for (int v = n + m + 1; v != 0; v = Eparent[v].from) {
        mn = Math.min(mn, Eparent[v].residualCapacity());
      }
      return mn;
    };

    Procedure augment = () -> {
      long b = bottleneck.getAsLong();
      for (int v = n + m + 1; v != 0; v = Eparent[v].from) {
        Edge e = Eparent[v];
        e.flow += b;
        e.rev.flow -= b;
    ...
    BooleanSupplier bfs = () -> {
      ...
      while (!queue.isEmpty()) {
        ...
        for (Edge e : adj[u]) {
          ...
          if (visited[v] != iteration[0] && e.residualCapacity() > 0) {
            visited[v] = iteration[0];
            Eparent[v] = e;
            queue.add(v);
    ...
    out.append(maxFlow).appendNewLine();
    for (Edge e : edgeList) {
      if (e.type == EdgeType.FORWARD && e.flow == 1) {
        out.append(e.from).append(" ").append(e.to - n).appendNewLine();
      }
    }
  }
...
@MultipleInheritanceDisallowed(inheritor = SchoolDance.class)
abstract class ModuleSignatures {
  ...
  class Edge {
    ...
    public long assignedCapacity;
    public long flow;
    public Edge rev;

    public Edge(int from, int to, long capacity, EdgeType type) {
      this.from = from;
      this.to = to;
    ...
    public long residualCapacity() {
      return this.assignedCapacity - this.flow;
    }
  }
}
