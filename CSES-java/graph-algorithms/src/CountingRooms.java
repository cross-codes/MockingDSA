...
public class CountingRooms extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) throws InterruptedException {
    new Thread(null, new CountingRooms(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    RecursionEmulator dfs = (y, x) -> {
      ...
      stack.push(new Item(y, x));
      while (!stack.isEmpty()) {
        var frame = stack.poll();
        visited[frame.y][frame.x] = true;
        for (int i = 0; i < 4; i++) {
          int X = frame.x + dx[i], Y = frame.y + dy[i];
    ...
    out.append(rooms).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = CountingRooms.class)
