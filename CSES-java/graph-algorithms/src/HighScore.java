...
public class HighScore extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new HighScore(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt(), m = in.nextInt();

    ArrayList<ArrayList<IntegerOrderedPair>> adj = new ArrayList<>(n), adjT = new ArrayList<>(n);
    for (int i = 0; i < n; i++) {
      adj.add(new ArrayList<>());
      adjT.add(new ArrayList<>());
    }

    ArrayList<IntegerOrderedTriplet> edges = new ArrayList<>();
    for (int i = 0; i < m; i++) {
      int a = in.nextInt() - 1, b = in.nextInt() - 1, x = in.nextInt();
      adj.get(a).add(new IntegerOrderedPair(b, -x));
      adjT.get(b).add(new IntegerOrderedPair(a, -x));
      edges.add(new IntegerOrderedTriplet(a, b, -x));
    }

    BitSet visited = new BitSet(n);
    DFS dfs = (instance, u) -> {
      visited.set(u);
      for (var pair : adj.get(u)) {
        if (!visited.get(pair.first)) {
          instance.run(instance, pair.first);
        }
      }
    };
    ...
    BitSet visitedT = new BitSet(n);
    DFS dfsR = (instance, u) -> {
      visitedT.set(u);
      for (var pair : adjT.get(u)) {
        if (!visitedT.get(pair.first)) {
          instance.run(instance, pair.first);
        }
      }
    };
    ...
    visited.and(visitedT);
    long[] distance = new long[n];
    Arrays.fill(distance, Long.MAX_VALUE >> 2);
    distance[0] = 0;
    for (int i = 1; i < n; i++) {
      for (var edge : edges) {
        int a = edge.first, b = edge.second, x = edge.third;
        if (visited.get(a) && visited.get(b)) {
          distance[b] = Math.min(distance[b], distance[a] + x);
        }
      }
    }
    ...
    for (var edge : edges) {
      int a = edge.first, b = edge.second, x = edge.third;
      if (visited.get(a) && visited.get(b) && distance[b] > distance[a] + x) {
        out.append(-1).appendNewLine();
        return;
      }
    }

    out.append(-distance[n - 1]).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = HighScore.class)
abstract class ModuleSignatures {
  @FunctionalInterface
  interface DFS {
    void run(DFS instance, int u);
  }
}
