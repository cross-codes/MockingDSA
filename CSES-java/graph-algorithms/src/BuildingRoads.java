...
public class BuildingRoads extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new BuildingRoads(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    IntegerArrayList compRep = new IntegerArrayList(n);
    for (int i = 0; i < n; i++) {
      if (!visited[i]) {
        compRep.add(i);
        dfs.run(i);
    ...
    out.append(compRep.size() - 1).appendNewLine();
    for (int i = 0; i < compRep.size() - 1; i++) {
      out.append(compRep.get(i) + 1).append(" ").append(compRep.get(i + 1) + 1).appendNewLine();
    }
  }

}

@MultipleInheritanceDisallowed(inheritor = BuildingRoads.class)
