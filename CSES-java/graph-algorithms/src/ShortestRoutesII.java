...
public class ShortestRoutesII extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new ShortestRoutesII(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    for (int y = 0; y < n; y++) {
      Arrays.fill(adjM[y], Integer.MAX_VALUE);
    }

    for (int i = 0; i < m; i++) {
      int a = in.nextInt(), b = in.nextInt(), c = in.nextInt();
      adjM[a - 1][b - 1] = Math.min(c, adjM[a - 1][b - 1]);
      adjM[b - 1][a - 1] = Math.min(c, adjM[b - 1][a - 1]);
    }
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (i == j) {
          distance[i][j] = 0;
        } else if (adjM[i][j] != Integer.MAX_VALUE) {
    ...
    for (int i = 0; i < q; i++) {
      ...
      out.append(distance[a][b] == Long.MAX_VALUE >> 2 ? -1 : distance[a][b]).appendNewLine();
    }

  }
}

@MultipleInheritanceDisallowed(inheritor = ShortestRoutesII.class)
