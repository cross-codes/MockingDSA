...
public class Labyrinth extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new Labyrinth(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    IntegerOrderedPair startPos = new IntegerOrderedPair(-1, -1), endPos = new IntegerOrderedPair(-1, -1);
    for (int y = 0; y < n; y++) {
      for (int x = 0; x < m; x++) {
    ...
    IntegerOrderedPair[][] parents = new IntegerOrderedPair[n][m];
    boolean[][] visited = new boolean[n][m];

    IntegerBiPredicate inBounds = (y, x) -> y < n && y >= 0 && x < m && x >= 0;

    boolean foundPath = false;
    Queue<IntegerOrderedPair> queue = new ArrayDeque<>();
    ...
    while (!queue.isEmpty()) {
      ...
      if (endPos.first == y && endPos.second == x) {
        foundPath = true;
        break;
      }
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (inBounds.test(Y, X) && grid[Y][X] != (byte) '#' && !visited[Y][X]) {
          visited[Y][X] = true;
          parents[Y][X] = pos;
          queue.add(new IntegerOrderedPair(Y, X));
        }
      }
    }
    ...
    IntegerArrayList trace = new IntegerArrayList(n * m);
    var curr = endPos;
    while (parents[curr.first][curr.second] != null) {
      var next = parents[curr.first][curr.second];
      trace.add(directionInv.get(curr, next));
      curr = next;
    }

    int sz = trace.size();
    out.append("YES ").append(sz).appendNewLine();
...
@MultipleInheritanceDisallowed(inheritor = Labyrinth.class)
