...
public class BuildingTeams extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new BuildingTeams(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    ColourFunction inverted = (c) -> (c == Colour.BLUE) ? Colour.RED : Colour.BLUE;

    Colour[] colour = new Colour[n];
    Arrays.fill(colour, Colour.GRAY);

    boolean[] possible = { true };

    RecursionEmulator dfs = (u, p) -> {

      class Item {
        public int s, p;

        public Item(int node, int parent) {
          this.s = node;
          this.p = parent;
        }
      }
      ...
      stack.push(new Item(u, p));
      while (!stack.isEmpty()) {
        Item i = stack.removeLast();
        if (i.p == -1) {
          colour[i.s] = Colour.BLUE;
        } else {
          colour[i.s] = inverted.get(colour[i.p]);
        }
        for (int e : adj[i.s]) {
          if (colour[e] == Colour.GRAY) {
            stack.add(new Item(e, i.s));
          } else if (colour[e] == colour[i.s]) {
            possible[0] = false;
          }
        }
      }
    ...
    for (int i = 0; i < n; i++) {
      out.append(colour[i] == Colour.BLUE ? 1 : 2).append(" ");
    }
...
@MultipleInheritanceDisallowed(inheritor = BuildingTeams.class)
