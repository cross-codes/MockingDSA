...
public class ShortestRoutesI extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new ShortestRoutesI(), "LaunchableDriver", 1048576L).start();
  }

  private void solveCase(int _case) {
    int n = in.nextInt(), m = in.nextInt();
    @SuppressWarnings("unchecked")
    ArrayList<LongOrderedPair>[] adj = new ArrayList[n];
    for (int i = 0; i < n; i++) {
      adj[i] = new ArrayList<>();
    }
    ...
    for (int i = 0; i < m; i++) {
      ...
      adj[a].add(new LongOrderedPair(b, c));
    }

    boolean[] processed = new boolean[n];
    long[] distance = new long[n];

    PriorityQueue<LongOrderedPair> queue = new PriorityQueue<>(n);
    Arrays.fill(distance, Long.MAX_VALUE >> 2);
    distance[0] = 0;
    queue.offer(new LongOrderedPair(0L, 0));
    while (!queue.isEmpty()) {
      int a = (int) queue.poll().second;
      ...
      for (var u : adj[a]) {
        long b = u.first;
        int w = (int) u.second;
        if (distance[a] + w < distance[(int) b]) {
          distance[(int) b] = distance[a] + w;
          queue.offer(new LongOrderedPair(distance[(int) b], b));
        }
      }
    }
...
@MultipleInheritanceDisallowed(inheritor = ShortestRoutesI.class)
