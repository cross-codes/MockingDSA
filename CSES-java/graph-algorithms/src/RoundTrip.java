...
public class RoundTrip extends ModuleSignatures implements Runnable {
  ...
  public static void main(String... args) {
    new Thread(null, new RoundTrip(), "LaunchableDriver", 1048576L).start();
  }
  ...
  private void solveCase(int _case) {
    ...
    Colour[] colour = new Colour[n];
    Arrays.fill(colour, Colour.WHITE);

    int[] parent = new int[n], cycleParams = { -1, -1 };
    Arrays.fill(parent, -1);

    RecursiveEmulator dfs = (u, par) -> {
      class Item {
        public int s, p, index;

        public Item(int node, int parent, int index) {
          this.s = node;
          this.p = parent;
          this.index = index;
        }
      ...
      stack.add(new Item(u, par, 0));

      while (!stack.isEmpty()) {
        Item f = stack.peekLast();

        if (f.index == 0) {
          colour[f.s] = Colour.GRAY;
        }

        if (f.index < adj[f.s].size()) {
          int v = adj[f.s].get(f.index);
          f.index += 1;
          ...
          if (colour[v] == Colour.WHITE) {
            parent[v] = f.s;
            stack.add(new Item(v, f.s, 0));
          } else if (colour[v] == Colour.GRAY && cycleParams[0] == -1) {
            cycleParams[0] = v;
            cycleParams[1] = f.s;
          }
        } else {
          colour[f.s] = Colour.BLACK;
          stack.removeLast();
        }
      }
    };
    ...
    IntegerArrayList cycle = new IntegerArrayList(n);
    int curr = cycleParams[1];

    while (curr != cycleParams[0]) {
      cycle.add(curr);
      curr = parent[curr];
    }

    out.append(cycle.size() + 2).appendNewLine();
    for (int v : cycle) {
      out.append(v + 1).append(" ");
    }

    out.append(cycleParams[0] + 1).append(" ").append(cycleParams[1] + 1).appendNewLine();
  }

}

@MultipleInheritanceDisallowed(inheritor = RoundTrip.class)
